...
public final class VectorizedMonoSplineKeyframesSpec<V extends AnimationVector> implements VectorizedDurationBasedAnimationSpec<V> {
    ...
    public VectorizedMonoSplineKeyframesSpec(IntList intList, IntObjectMap<Pair<V, Easing>> intObjectMap, int i, int i2, float f2) {
    ...
    private final float getEasedTimeFromIndex(int index, int timeMillis) {
        ...
        if (index >= this.timestamps._size - 1) {
            ...
            if (timeMillis != i) {
                float f3 = i2 - i;
                return ((f3 * getEasing(index).transform((timeMillis - i) / f3)) + i) / 1000;
            }
            f2 = i;
        }
        return f2 / 1000;
