...
public final class FlowLayoutBuildingBlocks {
    ...
    public final WrapEllipsisInfo getWrapEllipsisInfo(WrapInfo wrapInfo, boolean hasNext, int lastContentLineIndex, int totalCrossAxisSize, int leftOverMainAxis, int nextIndexInLine) {
        ...
        wrapEllipsisInfoEllipsisInfo$foundation_layout.setPlaceEllipsisOnLastContentLine(lastContentLineIndex >= 0 && (nextIndexInLine == 0 || (leftOverMainAxis - IntIntPair.m31getFirstimpl(wrapEllipsisInfoEllipsisInfo$foundation_layout.getEllipsisSize()) >= 0 && nextIndexInLine < this.maxItemsInMainAxis)));
    ...
    public final WrapInfo m988getWrapInfoOpUlnko(boolean nextItemHasNext, int nextIndexInLine, long leftOver, IntIntPair nextSize, int lineIndex, int totalCrossAxisSize, int currentLineCrossAxisSize, boolean isWrappingRound, boolean isEllipsisWrap) {
        int i = totalCrossAxisSize + currentLineCrossAxisSize;
        if (nextSize == null) {
            return new WrapInfo(true, true);
        }
        if (this.overflow.getType$foundation_layout() != FlowLayoutOverflow.OverflowType.Visible && (lineIndex >= this.maxLines || IntIntPair.m32getSecondimpl(leftOver) - IntIntPair.m32getSecondimpl(nextSize.getPackedValue()) < 0)) {
            return new WrapInfo(true, true);
        }
        if (nextIndexInLine != 0 && (nextIndexInLine >= this.maxItemsInMainAxis || IntIntPair.m31getFirstimpl(leftOver) - IntIntPair.m31getFirstimpl(nextSize.getPackedValue()) < 0)) {
            if (isWrappingRound) {
                return new WrapInfo(true, true);
            }
            return new WrapInfo(true, m988getWrapInfoOpUlnko(nextItemHasNext, 0, IntIntPair.m27constructorimpl(Constraints.m8783getMaxWidthimpl(this.constraints), (IntIntPair.m32getSecondimpl(leftOver) - this.crossAxisSpacing) - currentLineCrossAxisSize), IntIntPair.m24boximpl(IntIntPair.m27constructorimpl(IntIntPair.m31getFirstimpl(nextSize.getPackedValue()) - this.mainAxisSpacing, IntIntPair.m32getSecondimpl(nextSize.getPackedValue()))), lineIndex + 1, i, 0, true, false).getIsLastItemInContainer());
        }
        int iMax = totalCrossAxisSize + Math.max(currentLineCrossAxisSize, IntIntPair.m32getSecondimpl(nextSize.getPackedValue()));
        IntIntPair intIntPairM997ellipsisSizeF35zmw$foundation_layout = isEllipsisWrap ? null : this.overflow.m997ellipsisSizeF35zmw$foundation_layout(nextItemHasNext, lineIndex, iMax);
        if (intIntPairM997ellipsisSizeF35zmw$foundation_layout != null) {
            intIntPairM997ellipsisSizeF35zmw$foundation_layout.getPackedValue();
            if (nextIndexInLine + 1 >= this.maxItemsInMainAxis || ((IntIntPair.m31getFirstimpl(leftOver) - IntIntPair.m31getFirstimpl(nextSize.getPackedValue())) - this.mainAxisSpacing) - IntIntPair.m31getFirstimpl(intIntPairM997ellipsisSizeF35zmw$foundation_layout.getPackedValue()) < 0) {
                if (isEllipsisWrap) {
                    return new WrapInfo(true, true);
                }
                WrapInfo wrapInfoM988getWrapInfoOpUlnko = m988getWrapInfoOpUlnko(false, 0, IntIntPair.m27constructorimpl(Constraints.m8783getMaxWidthimpl(this.constraints), (IntIntPair.m32getSecondimpl(leftOver) - this.crossAxisSpacing) - Math.max(currentLineCrossAxisSize, IntIntPair.m32getSecondimpl(nextSize.getPackedValue()))), intIntPairM997ellipsisSizeF35zmw$foundation_layout, lineIndex + 1, iMax, 0, true, true);
                return new WrapInfo(wrapInfoM988getWrapInfoOpUlnko.getIsLastItemInContainer(), wrapInfoM988getWrapInfoOpUlnko.getIsLastItemInContainer());
            }
        }
