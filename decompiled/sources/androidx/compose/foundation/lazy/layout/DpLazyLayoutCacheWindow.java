...
final class DpLazyLayoutCacheWindow implements LazyLayoutCacheWindow {
    ...
    public /* synthetic */ DpLazyLayoutCacheWindow(float f2, float f3, DefaultConstructorMarker defaultConstructorMarker) {
        this(f2, f3);
    }

    private DpLazyLayoutCacheWindow(float f2, float f3) {
        this.ahead = f2;
        this.behind = f3;
    }
    ...
    public int calculateAheadWindow(Density density, int i) {
        return density.mo482roundToPx0680j_4(this.ahead);
    }

    @Override // androidx.compose.foundation.lazy.layout.LazyLayoutCacheWindow
    public int calculateBehindWindow(Density density, int i) {
        return density.mo482roundToPx0680j_4(this.behind);
    }

    public int hashCode() {
        return (Dp.m8836hashCodeimpl(this.ahead) * 31) + Dp.m8836hashCodeimpl(this.behind);
    }
    ...
    public boolean equals(Object other) {
        if (other instanceof DpLazyLayoutCacheWindow) {
            ...
            if (Dp.m8835equalsimpl0(dpLazyLayoutCacheWindow.ahead, this.ahead) && Dp.m8835equalsimpl0(dpLazyLayoutCacheWindow.behind, this.behind)) {
                return true;
            }
        }
        return false;
