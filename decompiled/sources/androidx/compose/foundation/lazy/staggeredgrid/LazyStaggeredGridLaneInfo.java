...
public final class LazyStaggeredGridLaneInfo {
    ...
    public final void setGaps(int itemIndex, int[] gaps) {
        int iSearchByIndex = searchByIndex(this.spannedItems, itemIndex);
        if (iSearchByIndex < 0) {
            if (gaps == null) {
                return;
            }
            this.spannedItems.add(-(iSearchByIndex + 1), new SpannedItem(itemIndex, gaps));
            return;
        }
        if (gaps == null) {
            this.spannedItems.remove(iSearchByIndex);
        } else {
            this.spannedItems.get(iSearchByIndex).setGaps(gaps);
        }
    }
