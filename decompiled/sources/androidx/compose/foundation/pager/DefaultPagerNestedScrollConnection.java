...
final class DefaultPagerNestedScrollConnection implements NestedScrollConnection {
    ...
    public final long m1459consumeOnOrientationQWom1Mo(long j, Orientation orientation) {
        if (orientation == Orientation.Vertical) {
            return Velocity.m9064copyOhffZ5M$default(j, 0.0f, 0.0f, 2, null);
        }
        return Velocity.m9064copyOhffZ5M$default(j, 0.0f, 0.0f, 1, null);
    }
    ...
    public long mo1255onPreScrollOzD1aCk(long available, int source) {
        ...
        if (NestedScrollSource.m7169equalsimpl0(source, NestedScrollSource.INSTANCE.m7181getUserInputWNlRxjI()) && Math.abs(this.state.getCurrentPageOffsetFraction()) > 1.0E-6d && Math.abs(m1458toFloatk4lQ0M(available)) > 0.0f) {
            ...
            float fCoerceIn = RangesKt.coerceIn(m1458toFloatk4lQ0M(available), currentPageOffsetFraction, pageSize);
            if (this.orientation == Orientation.Horizontal && layoutInfo.getReverseLayout() && ComposeFoundationFlags.isReverseLayoutNestedScrollConnectionInPagerFixEnabled) {
                fIntBitsToFloat = this.state.dispatchRawDelta(fCoerceIn);
            } else {
                fIntBitsToFloat = -this.state.dispatchRawDelta(-fCoerceIn);
            }
            ...
            return Offset.m5716copydBAh8RU(available, fIntBitsToFloat2, fIntBitsToFloat);
        }
        return Offset.INSTANCE.m5739getZeroF1C5BW0();
    }

    /* JADX INFO: renamed from: toFloat-k-4lQ0M, reason: not valid java name */
    private final float m1458toFloatk4lQ0M(long j) {
    ...
    public long mo677onPostScrollDzOQY0M(long consumed, long available, int source) {
        if (NestedScrollSource.m7169equalsimpl0(source, NestedScrollSource.INSTANCE.m7180getSideEffectWNlRxjI()) && m1457mainAxisk4lQ0M(available) != 0.0f) {
    ...
    public Object mo676onPostFlingRZ2iAVY(long j, long j2, Continuation<? super Velocity> continuation) {
        return Velocity.m9059boximpl(m1459consumeOnOrientationQWom1Mo(j2, this.orientation));
    }

    /* JADX INFO: renamed from: mainAxis-k-4lQ0M, reason: not valid java name */
    private final float m1457mainAxisk4lQ0M(long j) {
        return Float.intBitsToFloat((int) (this.orientation == Orientation.Horizontal ? j >> 32 : j & 4294967295L));
    }
}
