...
public final class PasswordInputTransformation implements InputTransformation {
    ...
    public void transformInput(TextFieldBuffer textFieldBuffer) {
        if (textFieldBuffer.getChanges().getChangeCount() != 1 || TextRange.m8269getLengthimpl(textFieldBuffer.getChanges().mo1926getRangejx7JFs(0)) != 1 || textFieldBuffer.hasSelection()) {
            setRevealCodepointIndex(-1);
            return;
        }
        int iM8271getMinimpl = TextRange.m8271getMinimpl(textFieldBuffer.getChanges().mo1926getRangejx7JFs(0));
        if (getRevealCodepointIndex$foundation() != iM8271getMinimpl) {
            this.scheduleHide.invoke();
            setRevealCodepointIndex(iM8271getMinimpl);
        }
    }
