...
public final class AndroidTextFieldKeyEventHandler extends TextFieldKeyEventHandler {
    ...
    public boolean mo1956onKeyEvent8zsqlwg(KeyEvent event, TransformedTextFieldState textFieldState, TextLayoutState textLayoutState, TextFieldSelectionState textFieldSelectionState, Function1<? super KeyCommand, ? extends Unit> clipboardKeyCommandsHandler, SoftwareKeyboardController keyboardController, boolean editable, boolean singleLine, Function0<Boolean> onSubmit) {
        if (KeyEventType.m7137equalsimpl0(KeyEvent_androidKt.m7145getTypeZmokQxo(event), KeyEventType.INSTANCE.m7141getKeyDownCS__XNY()) && event.isFromSource(257) && (!TextFieldKeyEventHandler_androidKt.m2100isFromSoftKeyboardZmokQxo(event) || !TextFieldKeyInput_androidKt.m1846isTypedEventZmokQxo(event))) {
            textFieldSelectionState.setInTouchMode(false);
        }
        return super.mo1956onKeyEvent8zsqlwg(event, textFieldState, textLayoutState, textFieldSelectionState, clipboardKeyCommandsHandler, keyboardController, editable, singleLine, onSubmit);
    }
}
