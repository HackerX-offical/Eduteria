...
public final class AndroidRippleIndicationInstance extends RippleIndicationInstance implements RememberObserver, RippleHostKey {
    ...
    private AndroidRippleIndicationInstance(boolean z, float f2, State<Color> state, State<RippleAlpha> state2, ViewGroup viewGroup) {
        super(z, state2);
        this.bounded = z;
        this.radius = f2;
        this.color = state;
        ...
        this.rippleSize = Size.INSTANCE.m5801getZeroNHjbRc();
        this.rippleRadius = -1;
    ...
    public void drawIndication(ContentDrawScope contentDrawScope) {
        int iRoundToInt;
        this.rippleSize = contentDrawScope.mo6549getSizeNHjbRc();
        if (Float.isNaN(this.radius)) {
            iRoundToInt = MathKt.roundToInt(RippleAnimationKt.m2392getRippleEndRadiuscSwnlzA(contentDrawScope, this.bounded, contentDrawScope.mo6549getSizeNHjbRc()));
        } else {
            iRoundToInt = contentDrawScope.mo482roundToPx0680j_4(this.radius);
        }
        this.rippleRadius = iRoundToInt;
        long jM5978unboximpl = this.color.getValue().m5978unboximpl();
        float pressedAlpha = this.rippleAlpha.getValue().getPressedAlpha();
        contentDrawScope.drawContent();
        ContentDrawScope contentDrawScope2 = contentDrawScope;
        m2397drawStateLayerH2RKhps(contentDrawScope2, this.radius, jM5978unboximpl);
        Canvas canvas = contentDrawScope2.getDrawContext().getCanvas();
        getInvalidateTick();
        RippleHostView rippleHostView = getRippleHostView();
        if (rippleHostView != null) {
            rippleHostView.m2396setRipplePropertiesbiQXAtU(contentDrawScope.mo6549getSizeNHjbRc(), this.rippleRadius, jM5978unboximpl, pressedAlpha);
    ...
    public void addRipple(PressInteraction.Press interaction, CoroutineScope scope) {
        ...
        rippleHostView.m2395addRippleKOepWvA(interaction, this.bounded, this.rippleSize, this.rippleRadius, this.color.getValue().m5978unboximpl(), this.rippleAlpha.getValue().getPressedAlpha(), this.onInvalidateRipple);
