...
public final class CarouselStateKt {
    ...
    public static final CarouselState rememberCarouselState(final int i, final Function0<Integer> function0, Composer composer, int i2, int i3) {
        ...
        CarouselState carouselState = (CarouselState) RememberSaveableKt.m5351rememberSaveable(objArr, (Saver) saver, (Function0) objRememberedValue, composer, 0);
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public static final Object animateScrollToPage(final LazyLayoutScrollScope lazyLayoutScrollScope, PagerState pagerState, int i, float f2, AnimationSpec<Float> animationSpec, Function2<? super ScrollScope, ? super Integer, Unit> function2, Continuation<? super Unit> continuation) {
        int iCoerceAtMost;
        ...
        float fCalculateScrollDistanceTo = calculateScrollDistanceTo(pagerState, pagerState.getCurrentPage(), i) + f2;
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public static final Unit animateScrollToPage$lambda$3(Ref.FloatRef floatRef, LazyLayoutScrollScope lazyLayoutScrollScope, float f2, float f3) {
    ...
    private static final float calculateScrollDistanceTo(PagerState pagerState, int i, int i2) {
        int iMo1474getViewportSizeYbymL2g = (int) (pagerState.getLayoutInfo().getOrientation() == Orientation.Horizontal ? pagerState.getLayoutInfo().mo1474getViewportSizeYbymL2g() >> 32 : pagerState.getLayoutInfo().mo1474getViewportSizeYbymL2g() & 4294967295L);
        return ((i2 - i) * (pagerState.getLayoutInfo().getPageSize() + pagerState.getLayoutInfo().getPageSpacing())) + (pagerState.getLayoutInfo().getSnapPosition().position(iMo1474getViewportSizeYbymL2g, pagerState.getLayoutInfo().getPageSize(), pagerState.getLayoutInfo().getBeforeContentPadding(), pagerState.getLayoutInfo().getAfterContentPadding(), i, pagerState.getPageCount()) - pagerState.getLayoutInfo().getSnapPosition().position(iMo1474getViewportSizeYbymL2g, pagerState.getLayoutInfo().getPageSize(), pagerState.getLayoutInfo().getBeforeContentPadding(), pagerState.getLayoutInfo().getAfterContentPadding(), i2, pagerState.getPageCount()));
    }
}
