...
public final class HctSolver {
    ...
    private final double trueDelinearized(double rgbComponent) {
        double d2 = rgbComponent / 100.0d;
        return (d2 <= 0.0031308d ? d2 * 12.92d : (Math.pow(d2, 0.4166666666666667d) * 1.055d) - 0.055d) * ((double) 255);
    }
    ...
    private final double hueOf(double[] linrgb) {
        ...
        double d4 = linrgb[1];
        double d5 = d3 + (dArr2[1] * d4);
        double d6 = linrgb[2];
        double d7 = d5 + (dArr2[2] * d6);
        double[] dArr3 = dArr[1];
        double d8 = (dArr3[0] * d2) + (dArr3[1] * d4) + (dArr3[2] * d6);
        double[] dArr4 = dArr[2];
        double d9 = (d2 * dArr4[0]) + (d4 * dArr4[1]) + (d6 * dArr4[2]);
        double dChromaticAdaptation = chromaticAdaptation(d7);
        double dChromaticAdaptation2 = chromaticAdaptation(d8);
        double dChromaticAdaptation3 = chromaticAdaptation(d9);
    ...
    private final boolean areInCyclicOrder(double a2, double b2, double c2) {
        return sanitizeRadians(b2 - a2) < sanitizeRadians(c2 - a2);
    }

    private final double[] lerpPoint(double[] source, double t, double[] target) {
        double d2 = source[0];
        double d3 = d2 + ((target[0] - d2) * t);
        double d4 = source[1];
        double d5 = d4 + ((target[1] - d4) * t);
        double d6 = source[2];
        return new double[]{d3, d5, d6 + ((target[2] - d6) * t)};
    }
    ...
    private final double[] nthVertex(double y, int n) {
        ...
        double d3 = dArr[1];
        double d4 = dArr[2];
        double d5 = n % 4 <= 1 ? 0.0d : 100.0d;
        double d6 = n % 2 != 0 ? 100.0d : 0.0d;
        if (n < 4) {
            double d7 = ((y - (d3 * d5)) - (d4 * d6)) / d2;
            if (isBounded(d7)) {
        ...
        if (n < 8) {
            double d8 = ((y - (d2 * d6)) - (d4 * d5)) / d3;
            if (isBounded(d8)) {
                return new double[]{d6, d8, d5};
            }
            return new double[]{-1.0d, -1.0d, -1.0d};
        }
        double d9 = ((y - (d2 * d5)) - (d3 * d6)) / d4;
        if (isBounded(d9)) {
        ...
        while (i < 12) {
            ...
            if (dArrNthVertex[c4] < 0.0d) {
                ...
                if (z) {
                    if (c3 == 0) {
                        d2 = d4;
                        if (hctSolver.areInCyclicOrder(d3, dHueOf, d4)) {
                        }
                    } else {
                        d2 = d4;
                    }
                    if (areInCyclicOrder(d3, targetHue, dHueOf)) {
                        d4 = dHueOf;
                        dArr2 = dArrNthVertex;
                        c3 = c4;
                    } else {
    ...
    private final int bisectToLimit(double y, double targetHue) {
        ...
        while (i2 < 3) {
            ...
            if (d2 == d3) {
                ...
                if (d2 < d3) {
                    iCriticalPlaneAbove = hctSolver.criticalPlaneBelow(hctSolver.trueDelinearized(d2));
                    iCriticalPlaneBelow = hctSolver.criticalPlaneAbove(hctSolver.trueDelinearized(dArr2[i2]));
                } else {
                    iCriticalPlaneAbove = hctSolver.criticalPlaneAbove(hctSolver.trueDelinearized(d2));
                ...
                while (i4 < 8 && Math.abs(i5 - i3) > 1.0d) {
                    ...
                    double d5 = d4;
                    if (hctSolver.areInCyclicOrder(d5, targetHue, dHueOf2)) {
                        d4 = d5;
                        i5 = iFloor;
                        dArr3 = coordinate;
                        dArr4 = dArr5;
        ...
        double d6 = 2;
        return CamUtils.INSTANCE.argbFromLinrgbComponents((dArr[0] + dArr2[0]) / d6, (dArr[1] + dArr2[1]) / d6, (dArr[2] + dArr2[2]) / d6);
    }

    private final double inverseChromaticAdaptation(double adapted) {
        double dAbs = Math.abs(adapted);
        return ((double) CamUtils.INSTANCE.signum(adapted)) * Math.pow(Math.max(0.0d, (27.13d * dAbs) / (400.0d - dAbs)), 2.380952380952381d);
    }
    ...
    private final int findResultByJ(double hueRadians, double chroma, double y) {
        ...
        char c2 = 1;
        double dPow = ((double) 1) / Math.pow(1.64d - Math.pow(0.29d, frame.getN()), 0.73d);
        ...
        while (i < 5) {
            char c3 = c2;
            double d2 = dPow;
            double d3 = dSqrt / 100.0d;
            int i2 = i;
            double d4 = dSqrt;
            double dPow2 = Math.pow(((chroma == 0.0d || dSqrt == 0.0d) ? 0.0d : chroma / Math.sqrt(d3)) * d2, 1.1111111111111112d);
            double aw = (((double) frame.getAw()) * Math.pow(d3, (1.0d / ((double) frame.getC())) / ((double) frame.getZ()))) / ((double) frame.getNbb());
            double d5 = (((0.305d + aw) * 23.0d) * dPow2) / (((23.0d * dCos) + ((((double) 11) * dPow2) * dCos2)) + ((108.0d * dPow2) * dSin));
            double d6 = d5 * dCos2;
            double d7 = d5 * dSin;
            double d8 = 460.0d * aw;
            double d9 = (((451.0d * d6) + d8) + (288.0d * d7)) / 1403.0d;
            Frame frame2 = frame;
            double d10 = ((d8 - (891.0d * d6)) - (261.0d * d7)) / 1403.0d;
            double d11 = ((d8 - (220.0d * d6)) - (6300.0d * d7)) / 1403.0d;
            double dInverseChromaticAdaptation = hctSolver.inverseChromaticAdaptation(d9);
            double dInverseChromaticAdaptation2 = hctSolver.inverseChromaticAdaptation(d10);
            double dInverseChromaticAdaptation3 = hctSolver.inverseChromaticAdaptation(d11);
            double[][] dArr = LINRGB_FROM_SCALED_DISCOUNT;
            double[] dArr2 = dArr[0];
            double d12 = (dArr2[0] * dInverseChromaticAdaptation) + (dArr2[c3] * dInverseChromaticAdaptation2) + (dArr2[2] * dInverseChromaticAdaptation3);
            double[] dArr3 = dArr[c3];
            double d13 = (dArr3[0] * dInverseChromaticAdaptation) + (dArr3[c3] * dInverseChromaticAdaptation2) + (dArr3[2] * dInverseChromaticAdaptation3);
            double[] dArr4 = dArr[2];
            double d14 = (dInverseChromaticAdaptation * dArr4[0]) + (dInverseChromaticAdaptation2 * dArr4[c3]) + (dInverseChromaticAdaptation3 * dArr4[2]);
            if (d12 >= 0.0d && d13 >= 0.0d && d14 >= 0.0d) {
                double[] dArr5 = Y_FROM_LINRGB;
                double d15 = (dArr5[0] * d12) + (dArr5[c3] * d13) + (dArr5[2] * d14);
                if (d15 <= 0.0d) {
                    return 0;
                }
                if (i2 != 4) {
                ...
                if (d12 <= 100.01d && d13 <= 100.01d && d14 <= 100.01d) {
                    return CamUtils.INSTANCE.argbFromLinrgbComponents(d12, d13, d14);
                }
            }
            return 0;
        }
