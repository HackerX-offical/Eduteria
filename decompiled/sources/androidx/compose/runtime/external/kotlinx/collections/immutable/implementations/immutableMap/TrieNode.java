...
public final class TrieNode<K, V> {
    ...
    private final TrieNode<K, V> mutableCollisionPutAll(TrieNode<K, V> otherNode, DeltaCounter intersectionCounter, MutabilityOwnership owner) {
        CommonFunctionsKt.m5287assert(this.nodeMap == 0);
        CommonFunctionsKt.m5287assert(this.dataMap == 0);
        CommonFunctionsKt.m5287assert(otherNode.nodeMap == 0);
        CommonFunctionsKt.m5287assert(otherNode.dataMap == 0);
        Object[] objArr = this.buffer;
    ...
    public final TrieNode<K, V> mutablePut(int keyHash, K key, V value, int shift, PersistentHashMapBuilder<K, V> mutator) {
        ...
        if (hasNodeAt(iIndexSegment)) {
            ...
            if (shift == 30) {
                ...
                persistentHashMapBuilder = mutator;
                trieNodeMutablePut = trieNodeNodeAtIndex$runtime.mutablePut(keyHash, key, value, shift + 5, persistentHashMapBuilder);
            }
            return trieNodeNodeAtIndex$runtime == trieNodeMutablePut ? this : mutableUpdateNodeAtIndex(iNodeIndex$runtime, trieNodeMutablePut, persistentHashMapBuilder.getOwnership());
        }
        mutator.setSize(mutator.size() + 1);
    ...
    public final TrieNode<K, V> remove(int keyHash, K key, V value, int shift) {
        ...
        if (hasEntryAt$runtime(iIndexSegment)) {
            int iEntryKeyIndex$runtime = entryKeyIndex$runtime(iIndexSegment);
            if (Intrinsics.areEqual(key, keyAtIndex(iEntryKeyIndex$runtime)) && Intrinsics.areEqual(value, valueAtKeyIndex(iEntryKeyIndex$runtime))) {
                return removeEntryAtIndex(iEntryKeyIndex$runtime, iIndexSegment);
            }
        } else if (hasNodeAt(iIndexSegment)) {
            int iNodeIndex$runtime = nodeIndex$runtime(iIndexSegment);
