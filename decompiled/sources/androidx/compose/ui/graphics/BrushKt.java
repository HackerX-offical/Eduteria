...
public final class BrushKt {
    public static final ShaderBrush toShaderBrush(Brush brush) {
        ...
        Brush brushM5923verticalGradient8A3gB4$default = Brush.Companion.m5923verticalGradient8A3gB4$default(Brush.INSTANCE, CollectionsKt.listOf((Object[]) new Color[]{Color.m5958boximpl(solidColor.getValue()), Color.m5958boximpl(solidColor.getValue())}), 0.0f, 0.0f, 0, 14, (Object) null);
        Intrinsics.checkNotNull(brushM5923verticalGradient8A3gB4$default, "null cannot be cast to non-null type androidx.compose.ui.graphics.ShaderBrush");
        return (ShaderBrush) brushM5923verticalGradient8A3gB4$default;
    }

    public static final List<Color> lerpColorList(List<Color> list, List<Color> list2, float f2) {
        int iMax = Math.max(list.size(), list2.size());
        ArrayList arrayList = new ArrayList(iMax);
        for (int i = 0; i < iMax; i++) {
            arrayList.add(Color.m5958boximpl(ColorKt.m6019lerpjxsXWHM(list.get(Math.min(i, list.size() - 1)).m5978unboximpl(), list2.get(Math.min(i, list2.size() - 1)).m5978unboximpl(), f2)));
        }
        return arrayList;
    }

    public static final List<Float> lerpNullableFloatList(List<Float> list, List<Float> list2, float f2) {
        if (list2 == null || list == null) {
            return null;
        }
        return lerpFloatList(list, list2, f2);
    }

    public static final List<Float> lerpFloatList(List<Float> list, List<Float> list2, float f2) {
        int iMax = Math.max(list.size(), list2.size());
        ArrayList arrayList = new ArrayList(iMax);
        for (int i = 0; i < iMax; i++) {
            arrayList.add(Float.valueOf(MathHelpersKt.lerp(list.get(Math.min(i, list.size() - 1)).floatValue(), list2.get(Math.min(i, list2.size() - 1)).floatValue(), f2)));
        }
        return arrayList;
    }

    /* JADX INFO: renamed from: lerpSafe-Wko1d7g, reason: not valid java name */
    public static final long m5936lerpSafeWko1d7g(long j, long j2, float f2) {
        if (((((j & androidx.compose.ui.geometry.InlineClassHelperKt.DualFloatInfinityBase) ^ androidx.compose.ui.geometry.InlineClassHelperKt.DualFloatInfinityBase) - androidx.compose.ui.geometry.InlineClassHelperKt.Uint64Low32) & (-9223372034707292160L)) == 0 && (((androidx.compose.ui.geometry.InlineClassHelperKt.DualFloatInfinityBase ^ (j2 & androidx.compose.ui.geometry.InlineClassHelperKt.DualFloatInfinityBase)) - androidx.compose.ui.geometry.InlineClassHelperKt.Uint64Low32) & (-9223372034707292160L)) == 0) {
            return OffsetKt.m5746lerpWko1d7g(j, j2, f2);
        }
        return f2 < 0.5f ? j : j2;
    }
