...
public final class Vertices {
    ...
    private final int[] encodeColorList(List<Color> colors) {
        ...
        for (int i = 0; i < size; i++) {
            iArr[i] = ColorKt.m6022toArgb8_81llA(colors.get(i).m5978unboximpl());
        }
        return iArr;
    }
    ...
    private final float[] encodePointList(List<Offset> points) {
        ...
        for (int i = 0; i < size; i++) {
            long jM5733unboximpl = points.get(i / 2).m5733unboximpl();
            if (i % 2 == 0) {
                fIntBitsToFloat = Float.intBitsToFloat((int) (jM5733unboximpl >> 32));
            } else {
                fIntBitsToFloat = Float.intBitsToFloat((int) (jM5733unboximpl & 4294967295L));
            }
