...
public final class ColorSpaces {
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public static final double Bt2020Hlg$lambda$0(double d2) {
        return INSTANCE.transferHlgOetf$ui_graphics(Bt2020HlgTransferParameters, d2);
    }

    /* JADX INFO: Access modifiers changed from: private */
    public static final double Bt2020Hlg$lambda$1(double d2) {
        return INSTANCE.transferHlgEotf$ui_graphics(Bt2020HlgTransferParameters, d2);
    }
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public static final double Bt2020Pq$lambda$0(double d2) {
        return INSTANCE.transferSt2048Oetf$ui_graphics(Bt2020PqTransferParameters, d2);
    }

    /* JADX INFO: Access modifiers changed from: private */
    public static final double Bt2020Pq$lambda$1(double d2) {
        return INSTANCE.transferSt2048Eotf$ui_graphics(Bt2020PqTransferParameters, d2);
    }
    ...
    public final ColorSpace match(float[] toXYZD50, TransferParameters function) {
        for (ColorSpace colorSpace : ColorSpacesArray) {
            if (ColorModel.m6400equalsimpl0(colorSpace.getModel(), ColorModel.INSTANCE.m6407getRgbxdoWZVw())) {
    ...
    public final double transferHlgOetf$ui_graphics(TransferParameters params, double x) {
        ...
        double f2 = (x * d2) / (params.getF() + 1.0d);
        if (f2 <= 1.0d) {
            dLog = a2 * Math.pow(f2, b2);
        } else {
            dLog = (c2 * Math.log(f2 - d3)) + e2;
        }
        return d2 * dLog;
    }
    ...
    public final double transferHlgEotf$ui_graphics(TransferParameters params, double x) {
        ...
        double e2 = params.getE();
        double f2 = params.getF() + 1.0d;
        double d5 = a2 * d3;
        if (d5 <= 1.0d) {
            dExp = Math.pow(d5, b2);
        } else {
            dExp = Math.exp((d3 - e2) * c2) + d4;
        }
        return f2 * d2 * dExp;
    }
    ...
    public final double transferSt2048Oetf$ui_graphics(TransferParameters params, double x) {
        double d2 = x < 0.0d ? -1.0d : 1.0d;
        double d3 = x * d2;
        double d4 = -params.getA();
        double d5 = params.getD();
        double f2 = 1.0d / params.getF();
        return d2 * Math.pow(Math.max(d4 + (d5 * Math.pow(d3, f2)), 0.0d) / (params.getB() + ((-params.getE()) * Math.pow(d3, f2))), 1.0d / params.getC());
    }

    public final double transferSt2048Eotf$ui_graphics(TransferParameters pq, double x) {
        double d2 = x < 0.0d ? -1.0d : 1.0d;
        double d3 = x * d2;
        return d2 * Math.pow(RangesKt.coerceAtLeast(pq.getA() + (pq.getB() * Math.pow(d3, pq.getC())), 0.0d) / (pq.getD() + (pq.getE() * Math.pow(d3, pq.getC()))), pq.getF());
    }
}
