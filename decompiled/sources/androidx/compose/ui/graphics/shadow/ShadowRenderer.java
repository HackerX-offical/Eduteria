...
public abstract class ShadowRenderer {
    ...
    private long shadowTintColor = Color.INSTANCE.m6004getUnspecified0d7_KjU();
    private long cornerRadius = CornerRadius.INSTANCE.m5695getZerokKHJgLs();
    private long generatedSize = Size.INSTANCE.m5800getUnspecifiedNHjbRc();
    ...
    protected abstract void mo6687buildShadow_SMYjrA(DrawScope drawScope, long j, long j2, Path path);

    /* JADX INFO: renamed from: onDrawShadow-MLmccfk */
    protected abstract void mo6688onDrawShadowMLmccfk(DrawScope drawScope, long j, long j2, Path path, float f2, ColorFilter colorFilter, Brush brush, int i);
    ...
    public final void m6698drawShadowerFMhIw(DrawScope drawScope, ColorFilter colorFilter, long j, long j2, Brush brush, float f2, int i) {
        updateParamsFromOutline(this.outline);
        if (colorFilter == null) {
            colorFilter = (brush != null || j2 == 16) ? null : m6697obtainTint8_81llA(j2);
        }
        ColorFilter colorFilter2 = colorFilter;
        long j3 = this.generatedSize;
        if (j3 == InlineClassHelperKt.UnspecifiedPackedFloats || !Size.m5788equalsimpl0(j3, j) || this.generatedLayoutDirection != drawScope.getLayoutDirection() || this.generatedDensity != drawScope.getDensity()) {
            mo6687buildShadow_SMYjrA(drawScope, j, this.cornerRadius, this.path);
            this.generatedSize = j;
            this.generatedLayoutDirection = drawScope.getLayoutDirection();
            this.generatedDensity = drawScope.getDensity();
        }
        mo6688onDrawShadowMLmccfk(drawScope, j, this.cornerRadius, this.path, f2, colorFilter2, brush, i);
    }

    public void invalidateShadow() {
        this.generatedSize = Size.INSTANCE.m5800getUnspecifiedNHjbRc();
    ...
    private final ColorFilter m6697obtainTint8_81llA(long color) {
        ColorFilter colorFilter = this.shadowTint;
        if (colorFilter != null && Color.m5969equalsimpl0(this.shadowTintColor, color)) {
            return colorFilter;
        }
        ColorFilter colorFilterM6009tintxETnrds$default = ColorFilter.Companion.m6009tintxETnrds$default(ColorFilter.INSTANCE, color, 0, 2, null);
        this.shadowTintColor = color;
        this.shadowTint = colorFilterM6009tintxETnrds$default;
        return colorFilterM6009tintxETnrds$default;
    }

    private final void updateParamsFromOutline(Outline outline) {
        if (outline instanceof Outline.Generic) {
            this.path = ((Outline.Generic) outline).getPath();
            this.cornerRadius = CornerRadius.INSTANCE.m5695getZerokKHJgLs();
            return;
        }
        if (outline instanceof Outline.Rounded) {
            ...
            if (RoundRectKt.isSimple(rounded.getRoundRect())) {
                this.path = null;
                this.cornerRadius = rounded.getRoundRect().m5773getTopLeftCornerRadiuskKHJgLs();
                return;
            } else {
