...
public abstract class HoverIconModifierNode extends Modifier.Node implements TraversableNode, PointerInputModifierNode, CompositionLocalConsumerModifierNode {
    ...
    public abstract boolean mo7216isRelevantPointerTypeuerMTgs(int pointerType);
    ...
    public void mo280onPointerEventH0pRuoY(PointerEvent pointerEvent, PointerEventPass pass, long bounds) {
        if (pass == PointerEventPass.Main) {
            ...
            for (int i = 0; i < size; i++) {
                if (mo7216isRelevantPointerTypeuerMTgs(changes.get(i).getType())) {
                    if (PointerEventType.m7252equalsimpl0(pointerEvent.getType(), PointerEventType.INSTANCE.m7256getEnter7fucELk())) {
                        onEnter();
                        return;
                    } else {
                        if (PointerEventType.m7252equalsimpl0(pointerEvent.getType(), PointerEventType.INSTANCE.m7257getExit7fucELk())) {
                            onExit();
                            return;
                        }
                        return;
    ...
    public long mo1913getTouchBoundsExpansionRZrCHBk() {
        DpTouchBoundsExpansion dpTouchBoundsExpansion = this.dpTouchBoundsExpansion;
        return dpTouchBoundsExpansion != null ? dpTouchBoundsExpansion.m7655roundToTouchBoundsExpansionTW6G1oQ(DelegatableNodeKt.requireDensity(this)) : TouchBoundsExpansion.INSTANCE.m7864getNoneRZrCHBk();
    }
