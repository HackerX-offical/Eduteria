...
public final class MultiParagraphKt {
    public static final int findParagraphByIndex(List<ParagraphInfo> list, int i) {
        ...
        while (true) {
            ...
            if (paragraphInfo.getStartIndex() > i) {
                ...
                b2 = paragraphInfo.getEndIndex() <= i ? (byte) -1 : (byte) 0;
            }
            if (b2 >= 0) {
                if (b2 <= 0) {
                    break;
    ...
    public static final CharSequence findParagraphByIndex$lambda$2$0(ParagraphInfo paragraphInfo) {
        return Constants.AES_PREFIX + paragraphInfo.getStartIndex() + ", " + paragraphInfo.getEndIndex() + ')';
    }
    ...
    public static final void m8114findParagraphsByRangeSbBc2M(List<ParagraphInfo> list, long j, Function1<? super ParagraphInfo, Unit> function1) {
        int size = list.size();
        for (int iFindParagraphByIndex = findParagraphByIndex(list, TextRange.m8271getMinimpl(j)); iFindParagraphByIndex < size; iFindParagraphByIndex++) {
            ParagraphInfo paragraphInfo = list.get(iFindParagraphByIndex);
            if (paragraphInfo.getStartIndex() >= TextRange.m8270getMaximpl(j)) {
                return;
            }
    ...
    public static final int findParagraphByLineIndex(List<ParagraphInfo> list, int i) {
        ...
        while (i2 <= size) {
            ...
            if (paragraphInfo.getStartLineIndex() > i) {
                ...
                b2 = paragraphInfo.getEndLineIndex() <= i ? (byte) -1 : (byte) 0;
            }
            if (b2 < 0) {
                i2 = i3 + 1;
            } else {
