    ...
    private final float computeMinIntrinsicWidth() {
        ...
        while (true) {
            ...
            if (priorityQueue.size() < 10) {
                ...
                if (intRange != null && intRange.getLast() - intRange.getFirst() < i - i2) {
                    priorityQueue.poll();
                    priorityQueue.add(new IntRange(i2, i));
                }
            }
        ...
        float desiredWidth = getDesiredWidth(intRange2.getFirst(), intRange2.getLast());
        while (it.hasNext()) {
            IntRange intRange3 = (IntRange) it.next();
            desiredWidth = Math.max(desiredWidth, getDesiredWidth(intRange3.getFirst(), intRange3.getLast()));
        }
        return desiredWidth;
    }
