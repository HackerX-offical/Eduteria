...
public final class EditingBuffer {
    ...
    private EditingBuffer(AnnotatedString annotatedString, long j) {
        ...
        this.selectionStart = TextRange.m8271getMinimpl(j);
        this.selectionEnd = TextRange.m8270getMaximpl(j);
        this.compositionStart = -1;
        this.compositionEnd = -1;
        int iM8271getMinimpl = TextRange.m8271getMinimpl(j);
        int iM8270getMaximpl = TextRange.m8270getMaximpl(j);
        if (iM8271getMinimpl < 0 || iM8271getMinimpl > annotatedString.length()) {
            throw new IndexOutOfBoundsException("start (" + iM8271getMinimpl + ") offset is outside of text region " + annotatedString.length());
        }
        if (iM8270getMaximpl < 0 || iM8270getMaximpl > annotatedString.length()) {
            throw new IndexOutOfBoundsException("end (" + iM8270getMaximpl + ") offset is outside of text region " + annotatedString.length());
        }
        if (iM8271getMinimpl > iM8270getMaximpl) {
            throw new IllegalArgumentException("Do not set reversed range: " + iM8271getMinimpl + " > " + iM8270getMaximpl);
        }
    }
    ...
    public final TextRange m8439getCompositionMzsxiRA$ui_text() {
        if (hasComposition$ui_text()) {
            return TextRange.m8261boximpl(TextRangeKt.TextRange(this.compositionStart, this.compositionEnd));
        }
        return null;
    }

    /* JADX INFO: renamed from: getSelection-d9O1mEE$ui_text, reason: not valid java name */
    public final long m8440getSelectiond9O1mEE$ui_text() {
    ...
    public final void delete$ui_text(int start, int end) {
        ...
        long jM8441updateRangeAfterDeletepWDy79M = EditingBufferKt.m8441updateRangeAfterDeletepWDy79M(TextRangeKt.TextRange(this.selectionStart, this.selectionEnd), jTextRange);
        setSelectionStart(TextRange.m8271getMinimpl(jM8441updateRangeAfterDeletepWDy79M));
        setSelectionEnd(TextRange.m8270getMaximpl(jM8441updateRangeAfterDeletepWDy79M));
        if (hasComposition$ui_text()) {
            long jM8441updateRangeAfterDeletepWDy79M2 = EditingBufferKt.m8441updateRangeAfterDeletepWDy79M(TextRangeKt.TextRange(this.compositionStart, this.compositionEnd), jTextRange);
            if (TextRange.m8267getCollapsedimpl(jM8441updateRangeAfterDeletepWDy79M2)) {
                commitComposition$ui_text();
            } else {
                this.compositionStart = TextRange.m8271getMinimpl(jM8441updateRangeAfterDeletepWDy79M2);
                this.compositionEnd = TextRange.m8270getMaximpl(jM8441updateRangeAfterDeletepWDy79M2);
            }
        }
    }
