...
public final class AndroidParagraphHelper_androidKt {
    ...
    public static final CharSequence createCharSequence(String str, float f2, TextStyle textStyle, List<? extends AnnotatedString.Range<? extends AnnotatedString.Annotation>> list, List<AnnotatedString.Range<Placeholder>> list2, Density density, Function4<? super FontFamily, ? super FontWeight, ? super FontStyle, ? super FontSynthesis, ? extends Typeface> function4, boolean z) {
        ...
        if (z && EmojiCompat.isConfigured()) {
            ...
            EmojiSupportMatch emojiSupportMatchM8093boximpl = (platformStyle == null || (paragraphStyle = platformStyle.getParagraphStyle()) == null) ? null : EmojiSupportMatch.m8093boximpl(paragraphStyle.getEmojiSupportMatch());
            strProcess = EmojiCompat.get().process(str, 0, str.length(), Integer.MAX_VALUE, emojiSupportMatchM8093boximpl == null ? 0 : EmojiSupportMatch.m8096equalsimpl0(emojiSupportMatchM8093boximpl.getValue(), EmojiSupportMatch.INSTANCE.m8100getAll_3YsG6Y()));
            Intrinsics.checkNotNull(strProcess);
        } else {
            strProcess = str;
        }
        if (list.isEmpty() && list2.isEmpty() && Intrinsics.areEqual(textStyle.getTextIndent(), TextIndent.INSTANCE.getNone()) && TextUnit.m9022getRawTypeimpl(textStyle.m8312getLineHeightXSAIIZE()) == 0) {
            return strProcess;
        }
        ...
        if (isIncludeFontPaddingEnabled(textStyle) && textStyle.getLineHeightStyle() == null) {
            SpannableExtensions_androidKt.m8561setLineHeightr9BaKPg(spannable, textStyle.m8312getLineHeightXSAIIZE(), f2, density);
            f3 = f2;
            density2 = density;
        } else {
            ...
            f3 = f2;
            density2 = density;
            SpannableExtensions_androidKt.m8560setLineHeightKmRG4DE(spannable, textStyle.m8312getLineHeightXSAIIZE(), f3, density2, lineHeightStyle);
        }
        SpannableExtensions_androidKt.setTextIndent(spannable, textStyle.getTextIndent(), f3, density2);
        SpannableExtensions_androidKt.setSpanStyles(spannable, textStyle, list, density2, function4);
        SpannableExtensions_androidKt.setBulletSpans(spannable, list, f3, density2, textStyle.getTextIndent());
