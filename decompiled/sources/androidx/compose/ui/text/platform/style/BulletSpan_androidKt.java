...
public final class BulletSpan_androidKt {
    ...
    public static final void setDrawStyle(Paint paint, DrawStyle drawStyle) {
        ...
        paint.setStrokeCap(DrawStyleSpan_androidKt.m8566toAndroidCapBeK7IIE(stroke.getCap()));
        paint.setStrokeJoin(DrawStyleSpan_androidKt.m8567toAndroidJoinWw9F2mQ(stroke.getJoin()));
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public static final void draw(Outline outline, Canvas canvas, Paint paint, float f2, float f3, int i) {
        if (outline instanceof Outline.Generic) {
            canvas.save();
            Outline.Generic generic = (Outline.Generic) outline;
            Rect rect = generic.getRect();
            canvas.translate(f2, f3 - ((rect.getBottom() - rect.getTop()) / 2.0f));
        ...
        if (outline instanceof Outline.Rounded) {
            ...
            if (!RoundRectKt.isSimple(rounded.getRoundRect())) {
                ...
                canvas.save();
                canvas.translate(f2, f3 - (rounded.getRoundRect().getHeight() / 2.0f));
                if (Path instanceof AndroidPath) {
                    canvas.drawPath(((AndroidPath) Path).getInternalPath(), paint);
                    canvas.restore();
                    return;
            ...
            float fIntBitsToFloat = Float.intBitsToFloat((int) (rounded.getRoundRect().m5773getTopLeftCornerRadiuskKHJgLs() >> 32));
            canvas.drawRoundRect(f2, f3 - (rounded.getRoundRect().getHeight() / 2.0f), (i * rounded.getRoundRect().getWidth()) + f2, (rounded.getRoundRect().getHeight() / 2.0f) + f3, fIntBitsToFloat, fIntBitsToFloat, paint);
            return;
        }
        ...
        float bottom = f3 - ((rect2.getBottom() - rect2.getTop()) / 2.0f);
        Rect rect3 = rectangle.getRect();
        float right = f2 + (i * (rect3.getRight() - rect3.getLeft()));
        Rect rect4 = rectangle.getRect();
        canvas.drawRect(f2, bottom, right, f3 + ((rect4.getBottom() - rect4.getTop()) / 2.0f), paint);
    }

    /* JADX INFO: Access modifiers changed from: private */
    /* JADX INFO: renamed from: setBrushAndDraw-yzxVdVo, reason: not valid java name */
    public static final void m8564setBrushAndDrawyzxVdVo(Paint paint, Brush brush, float f2, long j, Function0<Unit> function0) {
        ...
        if (brush instanceof SolidColor) {
            ...
            paint.setColor(ColorKt.m6022toArgb8_81llA(((SolidColor) brush).getValue()));
            function0.invoke();
        ...
        paint.setShader(((ShaderBrush) brush).mo5937createShaderuvyYCjk(j));
        function0.invoke();
