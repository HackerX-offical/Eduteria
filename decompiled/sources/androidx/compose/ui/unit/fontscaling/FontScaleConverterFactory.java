...
public final class FontScaleConverterFactory {
    ...
    static {
        ...
        if ((fontScaleConverterFactory.getScaleFromKey(sLookupTables.keyAt(0)) - 0.01f > 1.03f ? 1 : 0) == 0) {
    ...
    private final FontScaleConverter createInterpolatedTableBetween(FontScaleConverter start, FontScaleConverter end, float interpolationPoint) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            fArr2[i] = MathUtils.INSTANCE.lerp(start.convertSpToDp(f2), end.convertSpToDp(f2), interpolationPoint);
        }
    ...
    private final void put(float scaleKey, FontScaleConverter fontScaleConverter) {
        synchronized (LookupTablesWriteLock) {
            ...
            SparseArrayCompat<FontScaleConverter> sparseArrayCompatM37clone = sLookupTables.m37clone();
            fontScaleConverterFactory.putInto(sparseArrayCompatM37clone, scaleKey, fontScaleConverter);
            sLookupTables = sparseArrayCompatM37clone;
            Unit unit = Unit.INSTANCE;
        }
    }
