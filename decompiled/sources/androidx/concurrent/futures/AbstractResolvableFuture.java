...
public abstract class AbstractResolvableFuture<V> implements ListenableFuture<V> {
    ...
    volatile Listener f200listeners;
    ...
    static {
        ...
        try {
            synchronizedHelper = new SafeAtomicHelper(AtomicReferenceFieldUpdater.newUpdater(Waiter.class, Thread.class, Message.Thread.ELEMENT), AtomicReferenceFieldUpdater.newUpdater(Waiter.class, Waiter.class, ES6Iterator.NEXT_METHOD), AtomicReferenceFieldUpdater.newUpdater(AbstractResolvableFuture.class, Waiter.class, "waiters"), AtomicReferenceFieldUpdater.newUpdater(AbstractResolvableFuture.class, Listener.class, "listeners"), AtomicReferenceFieldUpdater.newUpdater(AbstractResolvableFuture.class, Object.class, "value"));
            th = null;
        } catch (Throwable th) {
            th = th;
    ...
    public final void addListener(Runnable runnable, Executor executor) {
        ...
        Listener listener = this.f200listeners;
    ...
    public String toString() {
        ...
        if (isCancelled()) {
            ...
            if (strPendingToString != null && !strPendingToString.isEmpty()) {
                sbAppend.append("PENDING, info=[").append(strPendingToString).append(Constants.AES_SUFFIX);
            } else if (isDone()) {
                addDoneString(sbAppend);
            } else {
        ...
        return sbAppend.append(Constants.AES_SUFFIX).toString();
    }
    ...
    protected String pendingToString() {
        Object obj = this.value;
        if (obj instanceof SetFuture) {
            return "setFuture=[" + userObjectToString(((SetFuture) obj).future) + Constants.AES_SUFFIX;
        }
    ...
    private void addDoneString(StringBuilder sb) {
        try {
            sb.append("SUCCESS, result=[").append(userObjectToString(getUninterruptibly(this))).append(Constants.AES_SUFFIX);
        } catch (CancellationException unused) {
            sb.append("CANCELLED");
        } catch (RuntimeException e2) {
            sb.append("UNKNOWN, cause=[").append(e2.getClass()).append(" thrown from get()]");
        } catch (ExecutionException e3) {
            sb.append("FAILURE, cause=[").append(e3.getCause()).append(Constants.AES_SUFFIX);
        }
    }
    ...
    private static final class SynchronizedHelper extends AtomicHelper {
        ...
        boolean casListeners(AbstractResolvableFuture<?> abstractResolvableFuture, Listener listener, Listener listener2) {
            synchronized (abstractResolvableFuture) {
                if (abstractResolvableFuture.f200listeners != listener) {
                    return false;
                }
                abstractResolvableFuture.f200listeners = listener2;
                return true;
            }
        }
