...
public abstract class TimeCycleSplineSet {
    ...
    public String toString() {
        ...
        for (int i = 0; i < this.count; i++) {
            str = str + Constants.AES_PREFIX + this.mTimePoints[i] + " , " + decimalFormat.format(this.mValues[i]) + "] ";
        }
        return str;
    }
    ...
    public static class CustomSet extends TimeCycleSplineSet {
        ...
        public boolean setProperty(MotionWidget motionWidget, float f2, long j, KeyCache keyCache) {
            ...
            this.last_cycle = (float) ((((double) this.last_cycle) + ((j2 * 1.0E-9d) * ((double) f3))) % 1.0d);
            this.last_time = j;
            float fCalcWave = calcWave(this.last_cycle);
            this.mContinue = false;
            for (int i = 0; i < this.mCache.length; i++) {
                this.mContinue |= ((double) this.mTempValues[i]) != 0.0d;
                this.mCache[i] = (this.mTempValues[i] * fCalcWave) + f4;
            }
