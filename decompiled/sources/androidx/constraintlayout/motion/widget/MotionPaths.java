...
class MotionPaths implements Comparable<MotionPaths> {
    ...
    void initCartesian(KeyPosition c2, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float f11 = f2;
        float f12 = (endTimePoint.x + (f5 / 2.0f)) - ((f6 / 2.0f) + f9);
        float f13 = (endTimePoint.y + (f7 / 2.0f)) - (f10 + (f8 / 2.0f));
        ...
        float f16 = Float.isNaN(c2.mPercentX) ? f11 : c2.mPercentX;
        float f17 = Float.isNaN(c2.mAltPercentY) ? 0.0f : c2.mAltPercentY;
        if (!Float.isNaN(c2.mPercentY)) {
            f11 = c2.mPercentY;
        }
        float f18 = Float.isNaN(c2.mAltPercentX) ? 0.0f : c2.mAltPercentX;
        this.mMode = 0;
        this.x = (int) (((startTimePoint.x + (f16 * f12)) + (f18 * f13)) - f14);
        this.y = (int) (((startTimePoint.y + (f12 * f17)) + (f13 * f11)) - f15);
    ...
    void initPath(KeyPosition c2, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float f11 = f2;
        float f12 = (endTimePoint.x + (endTimePoint.width / 2.0f)) - ((f8 / 2.0f) + f7);
        float f13 = (endTimePoint.y + (endTimePoint.height / 2.0f)) - ((f10 / 2.0f) + f9);
        float f14 = f12 * f11;
        ...
        float f18 = Float.isNaN(c2.mPercentY) ? 0.0f : c2.mPercentY;
        this.mMode = 1;
    ...
    void getCenter(double p, int[] toUse, double[] data, float[] point, double[] vdata, float[] velocity) {
        ...
        if (motionController != null) {
            ...
            double d2 = fSin;
            double d3 = fCos;
            fSin = (float) ((((double) f12) + (Math.sin(d3) * d2)) - ((double) (f3 / 2.0f)));
            fCos = (float) ((((double) f13) - (Math.cos(d3) * d2)) - ((double) (f4 / 2.0f)));
            double d4 = f5;
            double dSin = ((double) f14) + (Math.sin(d3) * d4);
            double d5 = f7;
            float fCos3 = (float) (dSin + (Math.cos(d3) * d5));
            fCos2 = (float) ((((double) f15) - (d4 * Math.cos(d3))) + (Math.sin(d3) * d5));
            f11 = fCos3;
        } else {
            f2 = 2.0f;
        }
        point[0] = fSin + (f3 / f2) + 0.0f;
        point[1] = fCos + (f4 / f2) + 0.0f;
        velocity[0] = f11;
        velocity[1] = fCos2;
    }
    ...
    void setView(float position, View view, int[] toUse, double[] data, double[] slope, double[] cycle, boolean mForceMeasure) {
        ...
        if (motionController != null) {
            ...
            double d3 = f3;
            double d4 = f4;
            float fSin = (float) ((((double) f15) + (Math.sin(d4) * d3)) - ((double) (f5 / 2.0f)));
            float fCos = (float) ((((double) f16) - (Math.cos(d4) * d3)) - ((double) (f6 / 2.0f)));
            double d5 = f8;
            double d6 = f9;
            float fSin2 = (float) (((double) f17) + (Math.sin(d4) * d5) + (Math.cos(d4) * d3 * d6));
            float fCos2 = (float) ((((double) f18) - (d5 * Math.cos(d4))) + (d3 * Math.sin(d4) * d6));
            if (slope.length >= 2) {
                slope[0] = fSin2;
                slope[1] = fCos2;
            }
