    ...
    */
    ...
    void processTouchEvent(MotionEvent event, MotionLayout.MotionTracker velocityTracker, int currentState, MotionScene motionScene) {
        ...
        if (action == 1) {
            ...
            float fAbs = f4 != 0.0f ? xVelocity / fArr2[c2] : yVelocity / fArr2[1];
        ...
        if (Math.abs((this.mTouchDirectionX * rawX) + (this.mTouchDirectionY * rawY)) > this.mDragThreshold || this.mDragStarted) {
            ...
            if (i3 != -1) {
                ...
                c4 = 0;
                f2 = progress2;
                this.mMotionLayout.getAnchorDpDt(i3, f2, this.mTouchAnchorX, this.mTouchAnchorY, this.mAnchorDpDt);
            } else {
                c3 = 1;
                f2 = progress2;
                c4 = 0;
            ...
            if (Math.abs(((f7 * fArr4[c4]) + (this.mTouchDirectionY * fArr4[c3])) * this.mDragScale) < 0.01d) {
                float[] fArr5 = this.mAnchorDpDt;
                fArr5[c4] = 0.01f;
                fArr5[c3] = 0.01f;
            }
            if (this.mTouchDirectionX != 0.0f) {
                f3 = rawX / this.mAnchorDpDt[c4];
            } else {
                f3 = rawY / this.mAnchorDpDt[c3];
            }
            float fMax = Math.max(Math.min(f2 + f3, 1.0f), 0.0f);
            ...
            if (fMax != progress3) {
                if (progress3 == 0.0f || progress3 == 1.0f) {
                    this.mMotionLayout.endTrigger(progress3 == 0.0f ? c3 : c4);
                }
                this.mMotionLayout.setProgress(fMax);
                velocityTracker.computeCurrentVelocity(1000);
                this.mMotionLayout.mLastVelocity = this.mTouchDirectionX != 0.0f ? velocityTracker.getXVelocity() / this.mAnchorDpDt[c4] : velocityTracker.getYVelocity() / this.mAnchorDpDt[c3];
            } else {
