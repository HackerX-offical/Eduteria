...
public class FlexBuffersBuilder {
    ...
    private Value createKeyVector(int i, int i2) {
        ...
        while (i3 < this.stack.size()) {
            int i4 = i3 + 1;
            iMax = Math.max(iMax, Value.elemWidth(4, 0, this.stack.get(i3).key, this.bb.writePosition(), i4));
            i3 = i4;
        }
    ...
    private static class Value {
        ...
        static Value float32(int i, float f2) {
            return new Value(i, 3, 2, f2);
        }

        static Value float64(int i, double d2) {
            return new Value(i, 3, 3, d2);
        }
