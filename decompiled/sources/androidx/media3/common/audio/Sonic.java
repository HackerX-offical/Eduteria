...
final class Sonic {
    ...
    public void queueEndOfStream() {
        ...
        float f3 = this.pitch;
        double d2 = f2 / f3;
        double d3 = this.rate * f3;
        int i3 = this.remainingInputToCopyFrameCount;
        int i4 = this.outputFrameCount + ((int) ((((((((double) (i2 - i3)) / d2) + ((double) i3)) + this.accumulatedSpeedAdjustmentError) + ((double) this.pitchFrameCount)) / d3) + 0.5d));
    ...
    private int skipPitchPeriod(short[] sArr, int i, double d2, int i2) {
        int i3;
        if (d2 >= 2.0d) {
            double d3 = (((double) i2) / (d2 - 1.0d)) + this.accumulatedSpeedAdjustmentError;
            int iRound = (int) Math.round(d3);
            this.accumulatedSpeedAdjustmentError = d3 - ((double) iRound);
            i3 = iRound;
        } else {
            double d4 = ((((double) i2) * (2.0d - d2)) / (d2 - 1.0d)) + this.accumulatedSpeedAdjustmentError;
    ...
    private int insertPitchPeriod(short[] sArr, int i, double d2, int i2) {
        int i3;
        if (d2 < 0.5d) {
            double d3 = ((((double) i2) * d2) / (1.0d - d2)) + this.accumulatedSpeedAdjustmentError;
            int iRound = (int) Math.round(d3);
            this.accumulatedSpeedAdjustmentError = d3 - ((double) iRound);
            i3 = iRound;
        } else {
            double d4 = ((((double) i2) * ((2.0d * d2) - 1.0d)) / (1.0d - d2)) + this.accumulatedSpeedAdjustmentError;
            int iRound2 = (int) Math.round(d4);
            this.remainingInputToCopyFrameCount = iRound2;
            this.accumulatedSpeedAdjustmentError = d4 - ((double) iRound2);
            i3 = i2;
        ...
        while (true) {
            if (this.remainingInputToCopyFrameCount > 0) {
                ...
                if (d2 > 1.0d) {
                    sonic = this;
                    d3 = d2;
                    iInsertPitchPeriod += iFindPitchPeriod + sonic.skipPitchPeriod(this.inputBuffer, iInsertPitchPeriod, d3, iFindPitchPeriod);
                } else {
                    sonic = this;
                    d3 = d2;
                    iInsertPitchPeriod += sonic.insertPitchPeriod(sonic.inputBuffer, iInsertPitchPeriod, d3, iFindPitchPeriod);
                }
            }
            if (sonic.maxRequiredFrameCount + iInsertPitchPeriod > i) {
