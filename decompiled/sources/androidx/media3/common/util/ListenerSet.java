...
public final class ListenerSet<T> {
    ...
    private final CopyOnWriteArraySet<ListenerHolder<T>> f215listeners;
    ...
    private ListenerSet(CopyOnWriteArraySet<ListenerHolder<T>> copyOnWriteArraySet, Looper looper, Clock clock, IterationFinishedEvent<T> iterationFinishedEvent, boolean z) {
        this.clock = clock;
        this.f215listeners = copyOnWriteArraySet;
    ...
    public ListenerSet<T> copy(Looper looper, Clock clock, IterationFinishedEvent<T> iterationFinishedEvent) {
        return new ListenerSet<>(this.f215listeners, looper, clock, iterationFinishedEvent, this.throwsWhenUsingWrongThread);
    }
    ...
    public void add(T t) {
        ...
        synchronized (this.releasedLock) {
            ...
            this.f215listeners.add(new ListenerHolder<>(t));
        }
    }

    public void remove(T t) {
        verifyCurrentThread();
        for (ListenerHolder<T> listenerHolder : this.f215listeners) {
            if (listenerHolder.listener.equals(t)) {
                listenerHolder.release(this.iterationFinishedEvent);
                this.f215listeners.remove(listenerHolder);
            }
        }
    }
    ...
    public void clear() {
        verifyCurrentThread();
        Iterator<ListenerHolder<T>> it = this.f215listeners.iterator();
        while (it.hasNext()) {
            it.next().release(this.iterationFinishedEvent);
        }
        this.f215listeners.clear();
    }

    public int size() {
        verifyCurrentThread();
        return this.f215listeners.size();
    }

    public void queueEvent(final int i, final Event<T> event) {
        verifyCurrentThread();
        final CopyOnWriteArraySet copyOnWriteArraySet = new CopyOnWriteArraySet(this.f215listeners);
