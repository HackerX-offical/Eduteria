...
final class MediaPeriodHolder {
    ...
    public MediaPeriodInfo f220info;
    ...
    public long getStartPositionRendererTime() {
        return this.f220info.startPositionUs + this.rendererPositionOffsetUs;
    }
    ...
    public boolean isFullyPreloaded() {
        if (this.prepared) {
            return isFullyBuffered() || getBufferedPositionUs() - this.f220info.startPositionUs >= this.targetPreloadBufferDurationUs;
        }
        return false;
    }
    ...
    public long getBufferedPositionUs() {
        if (!this.prepared) {
            return this.f220info.startPositionUs;
        }
        long bufferedPositionUs = this.hasEnabledTracks ? this.mediaPeriod.getBufferedPositionUs() : Long.MIN_VALUE;
        return bufferedPositionUs == Long.MIN_VALUE ? this.f220info.durationUs : bufferedPositionUs;
    }
    ...
    public void handlePrepared(float f2, Timeline timeline, boolean z) throws ExoPlaybackException {
        this.prepared = true;
        this.trackGroups = this.mediaPeriod.getTrackGroups();
        TrackSelectorResult trackSelectorResultSelectTracks = selectTracks(f2, timeline, z);
        long jMax = this.f220info.startPositionUs;
        if (this.f220info.durationUs != -9223372036854775807L && jMax >= this.f220info.durationUs) {
            jMax = Math.max(0L, this.f220info.durationUs - 1);
        }
        long jApplyTrackSelection = applyTrackSelection(trackSelectorResultSelectTracks, jMax, false);
        this.rendererPositionOffsetUs += this.f220info.startPositionUs - jApplyTrackSelection;
        this.f220info = this.f220info.copyWithStartPositionUs(jApplyTrackSelection);
    }
    ...
    public TrackSelectorResult selectTracks(float f2, Timeline timeline, boolean z) throws ExoPlaybackException {
        TrackSelectorResult trackSelectorResultSelectTracks = this.trackSelector.selectTracks(this.rendererCapabilities, getTrackGroups(), this.f220info.id, timeline);
    ...
    public void updateClipping() {
        if (this.mediaPeriod instanceof ClippingMediaPeriod) {
            ((ClippingMediaPeriod) this.mediaPeriod).updateClipping(0L, this.f220info.endPositionUs == -9223372036854775807L ? Long.MIN_VALUE : this.f220info.endPositionUs);
        }
    }
    ...
    public boolean canBeUsedForMediaPeriodInfo(MediaPeriodInfo mediaPeriodInfo) {
        return MediaPeriodQueue.areDurationsCompatible(this.f220info.durationUs, mediaPeriodInfo.durationUs) && this.f220info.startPositionUs == mediaPeriodInfo.startPositionUs && this.f220info.id.equals(mediaPeriodInfo.id);
    }
