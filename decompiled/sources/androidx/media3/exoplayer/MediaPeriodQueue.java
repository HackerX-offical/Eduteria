...
final class MediaPeriodQueue {
    ...
    public boolean shouldLoadNextMediaPeriod() {
        MediaPeriodHolder mediaPeriodHolder = this.loading;
        if (mediaPeriodHolder != null) {
            return !mediaPeriodHolder.f220info.isFinal && this.loading.isFullyBuffered() && this.loading.f220info.durationUs != -9223372036854775807L && this.length < 100;
        }
        return true;
    }
    ...
    public MediaPeriodHolder enqueueNextMediaPeriodHolder(MediaPeriodInfo mediaPeriodInfo) {
        MediaPeriodHolder mediaPeriodHolder = this.loading;
        long rendererOffset = mediaPeriodHolder == null ? 1000000000000L : (mediaPeriodHolder.getRendererOffset() + this.loading.f220info.durationUs) - mediaPeriodInfo.startPositionUs;
        ...
        if (mediaPeriodHolderRemovePreloadedMediaPeriodHolder == null) {
            ...
            mediaPeriodHolderRemovePreloadedMediaPeriodHolder.f220info = mediaPeriodInfo;
    ...
    public void invalidatePreloadPool(Timeline timeline) {
        ...
        Pair<Object, Long> defaultPeriodPositionOfNextWindow = getDefaultPeriodPositionOfNextWindow(timeline, mediaPeriodHolder.f220info.id.periodUid, 0L);
        if (defaultPeriodPositionOfNextWindow != null && !timeline.getWindow(timeline.getPeriodByUid(defaultPeriodPositionOfNextWindow.first, this.period).windowIndex, this.window).isLive()) {
            ...
            if (mediaPeriodHolderRemovePreloadedMediaPeriodHolder == null) {
                mediaPeriodHolderRemovePreloadedMediaPeriodHolder = this.mediaPeriodHolderFactory.create(mediaPeriodInfoForPeriodPosition, (mediaPeriodHolder.getRendererOffset() + mediaPeriodHolder.f220info.durationUs) - mediaPeriodInfoForPeriodPosition.startPositionUs);
            }
    ...
    private MediaPeriodInfo getMediaPeriodInfoForPeriodPosition(Timeline timeline, Object obj, long j, long j2) {
        ...
        return getMediaPeriodInfoForContent(timeline, mediaPeriodIdResolveMediaPeriodIdForAds.periodUid, j, -9223372036854775807L, mediaPeriodIdResolveMediaPeriodIdForAds.windowSequenceNumber);
    }
    ...
    public MediaPeriodHolder advancePlayingPeriod() {
        ...
        if (i == 0) {
            ...
            this.oldFrontPeriodWindowSequenceNumber = this.playing.f220info.id.windowSequenceNumber;
        }
    ...
    public void clear() {
        ...
        this.oldFrontPeriodWindowSequenceNumber = next.f220info.id.windowSequenceNumber;
        while (next != null) {
            next.release();
            next = next.getNext();
        }
    ...
    public boolean updateQueuedPeriods(Timeline timeline, long j, long j2) {
        ...
        while (next != null) {
            MediaPeriodInfo mediaPeriodInfo = next.f220info;
            if (mediaPeriodHolder == null) {
    ...
    */
    ...
    private void notifyQueueUpdate() {
        ...
        for (MediaPeriodHolder next = this.playing; next != null; next = next.getNext()) {
            builder.add(next.f220info.id);
        }
        MediaPeriodHolder mediaPeriodHolder = this.reading;
        final MediaSource.MediaPeriodId mediaPeriodId = mediaPeriodHolder == null ? null : mediaPeriodHolder.f220info.id;
        this.analyticsCollectorHandler.post(new Runnable() { // from class: androidx.media3.exoplayer.MediaPeriodQueue$$ExternalSyntheticLambda0
            ...
            public final void run() {
                this.f$0.m9191x6b40a91a(builder, mediaPeriodId);
            }
        });
    }

    /* JADX INFO: renamed from: lambda$notifyQueueUpdate$0$androidx-media3-exoplayer-MediaPeriodQueue, reason: not valid java name */
    /* synthetic */ void m9191x6b40a91a(ImmutableList.Builder builder, MediaSource.MediaPeriodId mediaPeriodId) {
    ...
    private long resolvePeriodUidToWindowSequenceNumber(Timeline timeline, Object obj) {
        ...
        for (MediaPeriodHolder next = this.playing; next != null; next = next.getNext()) {
            if (next.uid.equals(obj)) {
                return next.f220info.id.windowSequenceNumber;
            }
        }
        for (MediaPeriodHolder next2 = this.playing; next2 != null; next2 = next2.getNext()) {
            ...
            if (indexOfPeriod2 != -1 && timeline.getPeriod(indexOfPeriod2, this.period).windowIndex == i) {
                return next2.f220info.id.windowSequenceNumber;
            }
        }
    ...
    private long resolvePeriodUidToWindowSequenceNumberInPreloadPeriods(Object obj) {
        for (int i = 0; i < this.preloadPriorityList.size(); i++) {
            ...
            if (mediaPeriodHolder.uid.equals(obj)) {
                return mediaPeriodHolder.f220info.id.windowSequenceNumber;
            }
        }
        return -1L;
    }
    ...
    private boolean updateForPlaybackModeChange(Timeline timeline) {
        ...
        while (true) {
            ...
            while (((MediaPeriodHolder) Assertions.checkNotNull(next)).getNext() != null && !next.f220info.isLastInTimelinePeriod) {
                next = next.getNext();
            }
        ...
        next.f220info = getUpdatedMediaPeriodInfo(timeline2, next.f220info);
        return !zRemoveAfter;
    }
    ...
    private MediaPeriodInfo getFollowingMediaPeriodInfo(Timeline timeline, MediaPeriodHolder mediaPeriodHolder, long j) {
        MediaPeriodInfo mediaPeriodInfo = mediaPeriodHolder.f220info;
    ...
    private MediaPeriodInfo getFirstMediaPeriodInfoOfNextPeriod(Timeline timeline, MediaPeriodHolder mediaPeriodHolder, long j) {
        ...
        if (timeline.getWindow(i, this.window).firstPeriodIndex == nextPeriodIndex) {
            ...
            if (next != null && next.uid.equals(obj2)) {
                j3 = next.f220info.id.windowSequenceNumber;
            } else {
