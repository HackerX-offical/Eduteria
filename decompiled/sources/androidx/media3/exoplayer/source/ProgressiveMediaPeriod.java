...
final class ProgressiveMediaPeriod implements MediaPeriod, ExtractorOutput, Loader.Callback<ExtractingLoadable>, Loader.ReleaseCallback, SampleQueue.UpstreamFormatChangedListener {
    ...
    public ProgressiveMediaPeriod(Uri uri, DataSource dataSource, ProgressiveMediaExtractor progressiveMediaExtractor, DrmSessionManager drmSessionManager, DrmSessionEventListener.EventDispatcher eventDispatcher, LoadErrorHandlingPolicy loadErrorHandlingPolicy, MediaSourceEventListener.EventDispatcher eventDispatcher2, Listener listener, Allocator allocator, String str, int i, boolean z, long j, ReleasableExecutor releasableExecutor) {
        Loader loader;
        this.uri = uri;
    ...
    public void maybeThrowPrepareError() throws IOException {
        try {
            ...
            if (this.suppressPrepareError) {
                Log.e(TAG, "Suppressing preparation error because suppressPrepareError=true", e2);
                this.sampleQueuesBuilt = true;
                m9264x33ac0ff2(new SeekMap.Unseekable(-9223372036854775807L));
            } else {
                throw e2;
            }
        }
    ...
    int skipData(int i, long j) throws Throwable {
        if (suppressRead()) {
            return 0;
        }
        maybeNotifyDownstreamFormat(i);
    ...
    public void seekMap(final SeekMap seekMap) {
        this.handler.post(new Runnable() { // from class: androidx.media3.exoplayer.source.ProgressiveMediaPeriod$$ExternalSyntheticLambda3
            @Override // java.lang.Runnable
            public final void run() {
                this.f$0.m9264x33ac0ff2(seekMap);
            }
        });
    }
    ...
    public void m9264x33ac0ff2(SeekMap seekMap) {
    ...
    private boolean seekInsideBufferUs(boolean[] zArr, long j, boolean z) throws Throwable {
        boolean zSeekTo;
        int length = this.sampleQueues.length;
        for (int i = 0; i < length; i++) {
            SampleQueue sampleQueue = this.sampleQueues[i];
