...
public class EventLogger implements AnalyticsListener {
    ...
    public void onTimelineChanged(AnalyticsListener.EventTime eventTime, int i) {
        ...
        for (int i2 = 0; i2 < Math.min(periodCount, 3); i2++) {
            ...
            logd("  period [" + getTimeString(this.period.getDurationMs()) + Constants.AES_SUFFIX);
        }
        if (periodCount > 3) {
            logd("  ...");
        }
        for (int i3 = 0; i3 < Math.min(windowCount, 3); i3++) {
            ...
            logd("  window [" + getTimeString(this.window.getDurationMs()) + ", seekable=" + this.window.isSeekable + ", dynamic=" + this.window.isDynamic + Constants.AES_SUFFIX);
        }
        if (windowCount > 3) {
            logd("  ...");
        }
    ...
    public void onMediaItemTransition(AnalyticsListener.EventTime eventTime, MediaItem mediaItem, int i) {
        logd("mediaItem [" + getEventTimeString(eventTime) + ", reason=" + getMediaItemTransitionReasonString(i) + Constants.AES_SUFFIX);
    }
    ...
    public void onAudioAttributesChanged(AnalyticsListener.EventTime eventTime, AudioAttributes audioAttributes) {
        logd(eventTime, "audioAttributes", audioAttributes.contentType + Constants.SEPARATOR_COMMA + audioAttributes.flags + Constants.SEPARATOR_COMMA + audioAttributes.usage + Constants.SEPARATOR_COMMA + audioAttributes.allowedCapturePolicy);
    }
    ...
    private String getEventString(AnalyticsListener.EventTime eventTime, String str, String str2, Throwable th) {
        ...
        return str3 + Constants.AES_SUFFIX;
    }
    ...
    private static String getAudioTrackConfigString(AudioSink.AudioTrackConfig audioTrackConfig) {
        return audioTrackConfig.encoding + Constants.SEPARATOR_COMMA + audioTrackConfig.channelConfig + Constants.SEPARATOR_COMMA + audioTrackConfig.sampleRate + Constants.SEPARATOR_COMMA + audioTrackConfig.tunneling + Constants.SEPARATOR_COMMA + audioTrackConfig.offload + Constants.SEPARATOR_COMMA + audioTrackConfig.bufferSize;
    }
}
