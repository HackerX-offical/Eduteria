...
public final class AmrExtractor implements Extractor {
    ...
    public int read(ExtractorInput extractorInput, PositionHolder positionHolder) throws IOException {
        ...
        if (sample == -1) {
            ...
            if (seekMap instanceof IndexSeekMap) {
                ((IndexSeekMap) seekMap).setDurationUs(this.timeOffsetUs + this.currentSampleTimeUs);
    ...
    private void maybeOutputFormat() {
        ...
        this.currentTrackOutput.format(new Format.Builder().setSampleMimeType(z ? "audio/amr-wb" : "audio/3gpp").setMaxInputSize(z ? frameSizeBytesByTypeWb[8] : frameSizeBytesByTypeNb[7]).setChannelCount(1).setSampleRate(z ? 16000 : 8000).build());
    }
    ...
    private int getFrameSizeInBytes(int i) throws ParserException {
        ...
        throw ParserException.createForMalformedContainer("Illegal AMR " + (this.isWideBand ? "WB" : Constants.EASYPAY_PAYTYPE_NETBANKING) + " frame type " + i, null);
    }
    ...
    private void maybeOutputSeekMap(long j, int i) {
        ...
        if ((i3 & 4) != 0) {
            ...
            this.seekMap = getConstantBitrateSeekMap(j, (i3 & 2) != 0);
        }
        SeekMap seekMap = this.seekMap;
        if (seekMap != null) {
