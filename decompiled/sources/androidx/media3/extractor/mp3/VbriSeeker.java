...
final class VbriSeeker implements Seeker {
    ...
    public static VbriSeeker create(long j, long j2, MpegAudioUtil.Header header, ParsableByteArray parsableByteArray) {
        ...
        long jMax = j2 + ((long) header.frameSize) + ((long) parsableByteArray2.readInt());
        ...
        while (i3 < unsignedShort) {
            ...
            long j5 = unsignedByte;
            unsignedShort2 = i4;
            i3++;
            unsignedShort = unsignedShort;
            j3 = j4 + (j5 * ((long) unsignedShort2));
        ...
        if (jMax != j6) {
            Log.w(TAG, "VBRI bytes and ToC mismatch (using max): " + jMax + ", " + j6 + "\nSeeking will be inaccurate.");
            jMax = Math.max(jMax, j6);
        }
        return new VbriSeeker(jArr, jArr2, jScaleLargeTimestamp, jMax, header.bitrate);
    }
