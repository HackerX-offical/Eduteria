...
final class SsaStyle {
    ...
    private SsaStyle(String str, int i, Integer num, Integer num2, float f2, boolean z, boolean z2, boolean z3, boolean z4, int i2) {
        this.name = str;
        this.alignment = i;
    ...
    public static SsaStyle fromStyleLine(String str, Format format) {
        ...
        String[] strArrSplit = TextUtils.split(str.substring("Style:".length()), Constants.SEPARATOR_COMMA);
        ...
        try {
            ...
            int alignment = format.alignmentIndex != -1 ? parseAlignment(strArrSplit[format.alignmentIndex].trim()) : -1;
            Integer color = format.primaryColorIndex != -1 ? parseColor(strArrSplit[format.primaryColorIndex].trim()) : null;
            Integer color2 = format.outlineColorIndex != -1 ? parseColor(strArrSplit[format.outlineColorIndex].trim()) : null;
            float fontSize = format.fontSizeIndex != -1 ? parseFontSize(strArrSplit[format.fontSizeIndex].trim()) : -3.4028235E38f;
            boolean z3 = true;
            boolean z4 = false;
            if (format.boldIndex == -1 || !parseBooleanValue(strArrSplit[format.boldIndex].trim())) {
                z = true;
                z3 = false;
            } else {
                z = true;
            }
            if (format.italicIndex == -1 || !parseBooleanValue(strArrSplit[format.italicIndex].trim())) {
                z2 = false;
            } else {
                z2 = false;
                z4 = z;
            }
            if (format.underlineIndex != -1 && parseBooleanValue(strArrSplit[format.underlineIndex].trim())) {
                z2 = true;
            }
            return new SsaStyle(strTrim, alignment, color, color2, fontSize, z3, z4, z2, format.strikeoutIndex != -1 && parseBooleanValue(strArrSplit[format.strikeoutIndex].trim()), format.borderStyleIndex != -1 ? parseBorderStyle(strArrSplit[format.borderStyleIndex].trim()) : -1);
    ...
    static final class Format {
        ...
        public static Format fromFormatLine(String str) {
            String[] strArrSplit = TextUtils.split(str.substring("Format:".length()), Constants.SEPARATOR_COMMA);
            int i = -1;
            int i2 = -1;
            int i3 = -1;
            int i4 = -1;
