...
class FastScroller extends RecyclerView.ItemDecoration implements RecyclerView.OnItemTouchListener {
    ...
    void updateScrollPosition(int i, int i2) {
        ...
        if (z2) {
            float f2 = i3;
            this.mVerticalThumbCenterY = (int) ((f2 * (i2 + (f2 / 2.0f))) / iComputeVerticalScrollRange);
            this.mVerticalThumbHeight = Math.min(i3, (i3 * i3) / iComputeVerticalScrollRange);
        }
        if (this.mNeedHorizontalScrollbar) {
            float f3 = i4;
            this.mHorizontalThumbCenterX = (int) ((f3 * (i + (f3 / 2.0f))) / iComputeHorizontalScrollRange);
    ...
    boolean isPointInsideVerticalThumb(float f2, float f3) {
        if (isLayoutRTL()) {
            if (f2 > this.mVerticalThumbWidth) {
                return false;
            }
        } else if (f2 < this.mRecyclerViewWidth - this.mVerticalThumbWidth) {
            return false;
        }
