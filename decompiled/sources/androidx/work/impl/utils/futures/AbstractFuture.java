...
public abstract class AbstractFuture<V> implements ListenableFuture<V> {
    ...
    volatile Listener f243listeners;
    ...
    public final void addListener(Runnable listener, Executor executor) {
        ...
        Listener listener2 = this.f243listeners;
    ...
    private void addDoneString(StringBuilder builder) {
        try {
            builder.append("SUCCESS, result=[").append(userObjectToString(getUninterruptibly(this))).append(Constants.AES_SUFFIX);
        } catch (CancellationException unused) {
            builder.append("CANCELLED");
        } catch (RuntimeException e2) {
            builder.append("UNKNOWN, cause=[").append(e2.getClass()).append(" thrown from get()]");
        } catch (ExecutionException e3) {
            builder.append("FAILURE, cause=[").append(e3.getCause()).append(Constants.AES_SUFFIX);
        }
    }
    ...
    private static final class SynchronizedHelper extends AtomicHelper {
        ...
        boolean casListeners(AbstractFuture<?> future, Listener expect, Listener update) {
            synchronized (future) {
                if (future.f243listeners != expect) {
                    return false;
                }
                future.f243listeners = update;
                return true;
            }
        }
