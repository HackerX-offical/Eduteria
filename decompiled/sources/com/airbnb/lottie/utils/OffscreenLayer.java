...
public class OffscreenLayer {
    ...
    public Canvas start(Canvas canvas, RectF rectF, ComposeOp composeOp) {
        ...
        this.scaledBounds.set(rectF.left * f2, rectF.top * f3, rectF.right * f2, rectF.bottom * f3);
        ...
        int iOrdinal = this.currentStrategy.ordinal();
        if (iOrdinal == 0) {
            canvas.save();
            return canvas;
        }
        ...
        if (iOrdinal == 3) {
            ...
            recordingCanvasBeginRecording.scale(f2, f3);
    ...
    private void renderBitmapShadow(Canvas canvas, DropShadow dropShadow) {
        ...
        this.scaledRectIncludingShadow.set(rectFCalculateRectIncludingShadow.left * f2, rectFCalculateRectIncludingShadow.top * f3, rectFCalculateRectIncludingShadow.right * f2, rectFCalculateRectIncludingShadow.bottom * f3);
        ...
        this.shadowMaskBitmapCanvas.drawBitmap(this.bitmap, Math.round((this.targetRect.left - rectFCalculateRectIncludingShadow.left) * f2), Math.round((this.targetRect.top - rectFCalculateRectIncludingShadow.top) * f3), (Paint) null);
        if (this.shadowBlurFilter == null || this.lastShadowBlurRadius != dropShadow.getRadius()) {
            float radius = (dropShadow.getRadius() * (f2 + f3)) / 2.0f;
            if (radius > 0.0f) {
        ...
        this.shadowBitmapCanvas.drawBitmap(this.shadowMaskBitmap, Math.round(dropShadow.getDx() * f2), Math.round(dropShadow.getDy() * f3), this.shadowPaint);
    ...
    private void renderHardwareShadow(Canvas canvas, DropShadow dropShadow) {
        ...
        RectF rectF = new RectF(rectFCalculateRectIncludingShadow.left * f2, rectFCalculateRectIncludingShadow.top * f3, rectFCalculateRectIncludingShadow.right * f2, rectFCalculateRectIncludingShadow.bottom * f3);
        ...
        recordingCanvasBeginRecording.translate((-rectF.left) + (dropShadow.getDx() * f2), (-rectF.top) + (dropShadow.getDy() * f3));
