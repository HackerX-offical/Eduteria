...
public class TransferManager {
    ...
    private Upload doUpload(PutObjectRequest putObjectRequest, TransferStateChangeListener transferStateChangeListener, S3ProgressListener s3ProgressListener, PersistableUpload persistableUpload) throws AmazonClientException {
    ...
    public MultipleFileDownload downloadDirectory(String str, String str2, File file) {
        ...
        while (true) {
            ...
            while (true) {
                if (objectListingListNextBatchOfObjects == null) {
                    objectListingListNextBatchOfObjects = this.s3.listObjects(new ListObjectsRequest().withBucketName(str3).withDelimiter("/").withPrefix(str5));
                } else {
                ...
                for (S3ObjectSummary s3ObjectSummary : objectListingListNextBatchOfObjects.getObjectSummaries()) {
                    if (!s3ObjectSummary.getKey().equals(str5) && !objectListingListNextBatchOfObjects.getCommonPrefixes().contains(s3ObjectSummary.getKey() + "/")) {
                ...
                if (!objectListingListNextBatchOfObjects.isTruncated()) {
                    break;
                }
                str3 = str;
            }
        ...
        MultipleFileDownloadImpl multipleFileDownloadImpl = new MultipleFileDownloadImpl("Downloading from " + str3 + "/" + str4, transferProgress, progressListenerChain, str4, str3, arrayList);
    ...
    public MultipleFileUpload uploadFileList(String str, String str2, File file, List<File> list, ObjectMetadataProvider objectMetadataProvider) {
        ...
        MultipleFileTransferProgressUpdatingListener multipleFileTransferProgressUpdatingListener = new MultipleFileTransferProgressUpdatingListener(transferProgress, progressListenerChain);
        LinkedList linkedList = new LinkedList();
        MultipleFileUploadImpl multipleFileUploadImpl = new MultipleFileUploadImpl("Uploading etc", transferProgress, progressListenerChain, str6, str, linkedList);
        ...
        if (list == null || list.isEmpty()) {
            ...
            for (File file2 : list) {
                if (file2.isFile()) {
                    ...
                    str3 = str5;
                    linkedList.add((UploadImpl) doUpload(new PutObjectRequest(str, str6 + strReplaceAll, file2).withMetadata(objectMetadata).withGeneralProgressListener((ProgressListener) multipleFileTransferProgressUpdatingListener), multipleFileTransferStateChangeListener, null, null));
                } else {
                    str3 = str5;
                }
