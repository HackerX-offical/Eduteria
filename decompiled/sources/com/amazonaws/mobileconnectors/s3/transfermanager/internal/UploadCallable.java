...
public class UploadCallable implements Callable<UploadResult> {
    ...
    private UploadResult uploadInParts() throws Exception {
        ...
        try {
            try {
                ...
                UploadResult uploadResultUploadPartsInSeries = uploadPartsInSeries(uploadPartRequestFactory);
                if (inputStream != null) {
                    try {
                        return uploadResultUploadPartsInSeries;
                    } catch (Exception e3) {
                    }
                }
                return uploadResultUploadPartsInSeries;
            ...
            if (this.putObjectRequest.getInputStream() != null) {
                try {
                    ...
                    log.warn("Unable to cleanly close input stream: " + e32.getMessage(), e32);
                }
            }
        }
    }
    ...
    private UploadResult uploadPartsInSeries(UploadPartRequestFactory uploadPartRequestFactory) {
        ...
        while (uploadPartRequestFactory.hasMoreRequests()) {
            ...
            if (inputStream != null && inputStream.markSupported()) {
                if (nextUploadPartRequest.getPartSize() >= SieveCacheKt.NodeLinkMask) {
    ...
    private void uploadPartsInParallel(UploadPartRequestFactory uploadPartRequestFactory, String str) {
        Map<Integer, PartSummary> mapIdentifyExistingPartsForResume = identifyExistingPartsForResume(str);
        while (uploadPartRequestFactory.hasMoreRequests()) {
            if (this.threadPool.isShutdown()) {
                throw new CancellationException("TransferManager has been shutdown");
            }
            ...
            if (mapIdentifyExistingPartsForResume.containsKey(Integer.valueOf(nextUploadPartRequest.getPartNumber()))) {
                PartSummary partSummary = mapIdentifyExistingPartsForResume.get(Integer.valueOf(nextUploadPartRequest.getPartNumber()));
