...
public class AmazonS3EncryptionClient extends AmazonS3Client implements AmazonS3Encryption {
    public static final String USER_AGENT = AmazonS3EncryptionClient.class.getName() + MqttTopic.TOPIC_LEVEL_SEPARATOR + VersionInfoUtils.getVersion();
    ...
    public PutObjectResult putObject(PutObjectRequest putObjectRequest) {
        return this.crypto.putObjectSecurely(putObjectRequest.mo9801clone());
    }
    ...
    public void deleteObject(DeleteObjectRequest deleteObjectRequest) {
        ...
        DeleteObjectRequest deleteObjectRequest2 = (DeleteObjectRequest) deleteObjectRequest.mo9801clone();
    ...
    public CompleteMultipartUploadResult uploadObject(UploadObjectRequest uploadObjectRequest) throws ExecutionException, InterruptedException, IOException {
        ...
        UploadObjectObserver uploadObjectObserver2 = uploadObjectObserver;
        uploadObjectObserver2.init(uploadObjectRequest, new S3DirectImpl(), this, executorService2);
        String strOnUploadInitiation = uploadObjectObserver2.onUploadInitiation(uploadObjectRequest);
        ...
        try {
            try {
                try {
                    try {
                        multiFileOutputStream.init(uploadObjectObserver2, uploadObjectRequest.getPartSize(), uploadObjectRequest.getDiskLimit());
                        this.crypto.putLocalObjectSecurely(uploadObjectRequest, strOnUploadInitiation, multiFileOutputStream);
                        Iterator<Future<UploadPartResult>> it = uploadObjectObserver2.getFutures().iterator();
                        while (it.hasNext()) {
                        ...
                        return uploadObjectObserver2.onCompletion(arrayList);
                    } catch (IOException e2) {
                        throw ((IOException) onAbort(uploadObjectObserver2, e2));
                    }
                } catch (Error e3) {
                    throw ((Error) onAbort(uploadObjectObserver2, e3));
                } catch (InterruptedException e4) {
                    throw ((InterruptedException) onAbort(uploadObjectObserver2, e4));
                }
            } catch (RuntimeException e5) {
                throw ((RuntimeException) onAbort(uploadObjectObserver2, e5));
            } catch (ExecutionException e6) {
                throw ((ExecutionException) onAbort(uploadObjectObserver2, e6));
            }
        } catch (Throwable th) {
            if (z) {
                executorService2.shutdownNow();
