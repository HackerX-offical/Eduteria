...
final class ContentCryptoMaterial {
    ...
    private static ContentCryptoMaterial fromObjectMetadata0(ObjectMetadata objectMetadata, EncryptionMaterialsAccessor encryptionMaterialsAccessor, Provider provider, long[] jArr, ExtraMaterialsDescription extraMaterialsDescription, boolean z, AWSKMSClient aWSKMSClient) {
        ...
        return new ContentCryptoMaterial(mapMergeInto, bArrDecode, str3, contentCryptoSchemeFromCEKAlgo.createCipherLite(cek(bArrDecode, str3, encryptionMaterials, provider, contentCryptoSchemeFromCEKAlgo, aWSKMSClient), bArrDecode2, 2, provider));
    }
    ...
    private static ContentCryptoMaterial fromInstructionFile0(Map<String, String> map, EncryptionMaterialsAccessor encryptionMaterialsAccessor, Provider provider, long[] jArr, ExtraMaterialsDescription extraMaterialsDescription, boolean z, AWSKMSClient aWSKMSClient) {
        ...
        byte[] bArrDecode = Base64.decode(str);
        byte[] bArrDecode2 = Base64.decode(map.get(Headers.CRYPTO_IV));
        if (bArrDecode == null || bArrDecode2 == null) {
        ...
        Map<String, String> mapMatdescFromJson = matdescFromJson(map.get(Headers.MATERIALS_DESCRIPTION));
        ...
        if (z2) {
            ...
            int tagLengthInBits = contentCryptoSchemeFromCEKAlgo.getTagLengthInBits();
            if (tagLengthInBits > 0 && tagLengthInBits != (i = Integer.parseInt(map.get(Headers.CRYPTO_TAG_LENGTH)))) {
                throw new AmazonClientException("Unsupported tag length: " + i + ", expected: " + tagLengthInBits);
            }
        }
        if (z && str2 == null) {
        ...
        return new ContentCryptoMaterial(mapMergeInto, bArrDecode, str2, contentCryptoSchemeFromCEKAlgo.createCipherLite(cek(bArrDecode, str2, encryptionMaterials, provider, contentCryptoSchemeFromCEKAlgo, aWSKMSClient), bArrDecode2, 2, provider));
    }
