...
public class CryptoModuleDispatcher extends S3CryptoModule<MultipartUploadContext> {
    ...
    public CryptoModuleDispatcher(AWSKMSClient aWSKMSClient, S3Direct s3Direct, AWSCredentialsProvider aWSCredentialsProvider, EncryptionMaterialsProvider encryptionMaterialsProvider, CryptoConfiguration cryptoConfiguration) {
        CryptoConfiguration cryptoConfigurationMo9803clone = cryptoConfiguration.mo9803clone();
        if (cryptoConfigurationMo9803clone.getCryptoMode() == null) {
            cryptoConfigurationMo9803clone.setCryptoMode(CryptoMode.EncryptionOnly);
        }
        CryptoConfiguration only = cryptoConfigurationMo9803clone.readOnly();
        ...
        if (i == 1) {
            ...
            if (i == 3) {
                ...
                CryptoConfiguration cryptoConfigurationMo9803clone2 = only.mo9803clone();
                try {
                    cryptoConfigurationMo9803clone2.setCryptoMode(CryptoMode.AuthenticatedEncryption);
                } catch (UnsupportedOperationException unused) {
                }
                this.ae = new S3CryptoModuleAE(aWSKMSClient, s3Direct, aWSCredentialsProvider, encryptionMaterialsProvider, cryptoConfigurationMo9803clone2.readOnly());
                return;
            }
