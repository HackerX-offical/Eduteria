...
public class LiveClasses extends Fragment implements NetworkCall.MyNetworkCallBack {
    private NonScrollRecyclerView LiveClassesrecycler;
    Button backBtn;
    ...
    public static LiveClasses newInstance(String param1, String param2) {
        LiveClasses liveClasses = new LiveClasses();
        liveClasses.setArguments(new Bundle());
        return liveClasses;
    }
    ...
    public void onViewCreated(View view, Bundle savedInstanceState) {
        ...
        this.LiveClassesrecycler = (NonScrollRecyclerView) view.findViewById(R.id.Liveclassesrecycler);
        ...
        if (this.livevideocompleted.size() == 0) {
            ...
            this.LiveClassesrecycler.setVisibility(0);
            Liveclassadapter liveclassadapter = new Liveclassadapter(getActivity(), Helper.CheckLiveStatus(this.livevideocompleted, 0), this.time, "live");
            this.liveclassadapter = liveclassadapter;
            this.LiveClassesrecycler.setAdapter(liveclassadapter);
        }
        this.pullToReferesh.setOnRefreshListener(new SwipeRefreshLayout.OnRefreshListener() { // from class: com.appnew.android.LiveClass.Fragment.LiveClasses.1
            @Override // androidx.swiperefreshlayout.widget.SwipeRefreshLayout.OnRefreshListener
            public void onRefresh() {
                LiveClasses.this.initialState();
                LiveClasses.this.status = false;
                Helper.showProgressDialog(LiveClasses.this.getActivity());
                LiveClasses.this.livevideocompleted = new ArrayList<>();
                LiveClasses.this.hit_api_for_data(false);
                LiveClasses.this.pullToReferesh.setRefreshing(false);
            }
        });
        this.nestedScrollView.setOnScrollChangeListener(new NestedScrollView.OnScrollChangeListener() { // from class: com.appnew.android.LiveClass.Fragment.LiveClasses.2
            @Override // androidx.core.widget.NestedScrollView.OnScrollChangeListener
            public void onScrollChange(NestedScrollView v, int scrollX, int scrollY, int oldScrollX, int oldScrollY) {
                if (v.getChildAt(v.getChildCount() - 1) == null || scrollY < v.getChildAt(v.getChildCount() - 1).getMeasuredHeight() - v.getMeasuredHeight() || scrollY <= oldScrollY || !LiveClasses.this.loading || !LiveClasses.this.isPaginationAvailable) {
                    return;
                }
                LiveClasses.this.paginationLoader.setVisibility(0);
                LiveClasses.this.mPage++;
                LiveClasses.this.status = true;
                LiveClasses.this.hit_api_for_data(false);
            }
        });
    }
            ...
            try {
                ...
                if (jsonstring.getString("status").equalsIgnoreCase("true")) {
                    ...
                    if (liveClassesData2.getData() != null) {
                        ...
                        this.LiveClassesrecycler.setVisibility(8);
                        return;
                    }
                    this.no_data_found_RL.setVisibility(0);
                    this.LiveClassesrecycler.setVisibility(8);
    ...
    public void ErrorCallBack(String jsonstring, String apitype, String typeApi) {
        ...
        if (apitype.equals(API.get_liveclasses_data)) {
            ...
            NonScrollRecyclerView nonScrollRecyclerView = this.LiveClassesrecycler;
    ...
    public void checkVideoId(String number, Completed completedClassFragment) {
        ...
        Iterator<Datum> it = this.liveClasses.getData().iterator();
        int i = 0;
        while (it.hasNext()) {
            if (it.next().getId().equalsIgnoreCase(number)) {
                completedClassFragment.updateApi();
