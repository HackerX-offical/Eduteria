...
class SourceGenerator implements DataFetcherGenerator, DataFetcherGenerator.FetcherReadyCallback {
    ...
    private final DecodeHelper<?> f559helper;
    ...
    public boolean startNext() {
        ...
        while (!z && hasNextModelLoader()) {
            List<ModelLoader.LoadData<?>> loadData = this.f559helper.getLoadData();
            int i = this.loadDataListIndex;
            this.loadDataListIndex = i + 1;
            this.loadData = loadData.get(i);
            if (this.loadData != null && (this.f559helper.getDiskCacheStrategy().isDataCacheable(this.loadData.fetcher.getDataSource()) || this.f559helper.hasLoadPath(this.loadData.fetcher.getDataClass()))) {
    ...
    private void startNextLoad(final ModelLoader.LoadData<?> loadData) {
        this.loadData.fetcher.loadData(this.f559helper.getPriority(), new DataFetcher.DataCallback<Object>() { // from class: com.bumptech.glide.load.engine.SourceGenerator.1
    ...
    private boolean hasNextModelLoader() {
        return this.loadDataListIndex < this.f559helper.getLoadData().size();
    }
    ...
    private boolean cacheData(Object obj) throws Throwable {
        ...
        boolean z = false;
        try {
            DataRewinder<T> rewinder = this.f559helper.getRewinder(obj);
            Object objRewindAndGet = rewinder.rewindAndGet();
            Encoder<X> sourceEncoder = this.f559helper.getSourceEncoder(objRewindAndGet);
            DataCacheWriter dataCacheWriter = new DataCacheWriter(sourceEncoder, objRewindAndGet, this.f559helper.getOptions());
            DataCacheKey dataCacheKey = new DataCacheKey(this.loadData.sourceKey, this.f559helper.getSignature());
            DiskCache diskCache = this.f559helper.getDiskCache();
            ...
            if (diskCache.get(dataCacheKey) != null) {
                ...
                this.sourceCacheGenerator = new DataCacheGenerator(Collections.singletonList(this.loadData.sourceKey), this.f559helper, this);
    ...
    void onDataReadyInternal(ModelLoader.LoadData<?> loadData, Object obj) {
        DiskCacheStrategy diskCacheStrategy = this.f559helper.getDiskCacheStrategy();
