...
public class CleverTapInstanceConfig implements Parcelable {
    ...
    public static CleverTapInstanceConfig getDefaultInstance(Context context) {
        ...
        return createInstanceWithManifest(manifestInfo, accountId, accountToken, manifestInfo.getAccountRegion(), true);
    }
    ...
    private CleverTapInstanceConfig(ManifestInfo manifestInfo, String str, String str2, String str3, boolean z) {
        ...
        this.encryptionInTransit = encryptionInTransit == null ? "0" : encryptionInTransit;
    }
    ...
    private void buildPushProvidersFromManifest(ManifestInfo manifestInfo) {
        ...
        try {
            ...
            if (vendorOneProvider != null && (strArrSplit2 = vendorOneProvider.split(Constants.SEPARATOR_COMMA)) != null && strArrSplit2.length == 4) {
                addPushType(new PushType(strArrSplit2[0].trim(), strArrSplit2[1].trim(), strArrSplit2[2].trim(), strArrSplit2[3].trim()));
            }
            String vendorTwoProvider = manifestInfo.getVendorTwoProvider();
            if (vendorTwoProvider == null || (strArrSplit = vendorTwoProvider.split(Constants.SEPARATOR_COMMA)) == null || strArrSplit.length != 4) {
                return;
            }
            addPushType(new PushType(strArrSplit[0].trim(), strArrSplit[1].trim(), strArrSplit[2].trim(), strArrSplit[3].trim()));
        ...
        try {
            ...
            if (jSONObject.has(KEY_PUSH_TYPES)) {
                ...
                for (int i = 0; i < jSONArray.length(); i++) {
                    PushType pushTypeFromJSONObject = PushType.fromJSONObject(jSONArray.getJSONObject(i));
                    if (pushTypeFromJSONObject != null) {
                        addPushType(pushTypeFromJSONObject);
                    }
                }
            }
    ...
    private JSONArray getPushTypesArray() {
        ...
        for (PushType pushType : getPushTypes()) {
            if (pushType != PushConstants.FCM) {
                jSONArray.put(pushType.toJSONObject());
            }
        }
        return jSONArray;
    }
