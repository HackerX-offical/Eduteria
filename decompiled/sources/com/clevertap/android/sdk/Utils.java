...
public final class Utils {
        ...
        if (jSONArray != null) {
            for (int i = 0; i < jSONArray.length(); i++) {
                try {
                    ...
                    Logger.v("Could not convert JSONArray to ArrayList - " + e2.getMessage());
                }
            }
        }
        return arrayList;
    ...
    private static DownloadedBitmap downloadGif(String str, Context context, CleverTapInstanceConfig cleverTapInstanceConfig) {
        ...
        if (str != null) {
            ...
            cleverTapInstanceConfig2.getLogger().debug(cleverTapInstanceConfig2.getAccountId(), "Couldn't download gif for notification: " + e.getMessage());
        }
        return null;
    }
    ...
    private static Uri saveGifToFileAndGetUri(byte[] bArr, Context context, CleverTapInstanceConfig cleverTapInstanceConfig, Clock clock) {
        try {
            ...
            cleverTapInstanceConfig.getLogger().debug(cleverTapInstanceConfig.getAccountId(), "Failed to write gif to file or create URI: " + e2);
            return null;
        }
    }
    ...
    public static Uri getNotificationGifURI(String str, Context context, CleverTapInstanceConfig cleverTapInstanceConfig, Clock clock) {
        DownloadedBitmap downloadedBitmapDownloadGif = downloadGif(str, context, cleverTapInstanceConfig);
        if (downloadedBitmapDownloadGif == null) {
            return null;
        }
        return saveGifToFileAndGetUri(downloadedBitmapDownloadGif.getBytes(), context, cleverTapInstanceConfig, clock);
    }
    ...
    public static void cleanupOldGIFs(Context context, CleverTapInstanceConfig cleverTapInstanceConfig, Clock clock) {
        ...
        if (dir != null) {
            try {
                ...
                cleverTapInstanceConfig.getLogger().debug(cleverTapInstanceConfig.getAccountId(), "Error during animated image cleanup: " + e2.getMessage());
            }
        }
    }
