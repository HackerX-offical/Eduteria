...
public class CTFeatureFlagsController {
    ...
    public synchronized void updateFeatureFlags(JSONObject jSONObject) throws JSONException {
        ...
        for (int i = 0; i < jSONArray.length(); i++) {
            try {
                ...
                this.store.put(jSONObject2.getString(CTProductConfigConstants.PRODUCT_CONFIG_JSON_KEY_FOR_KEY), Boolean.valueOf(jSONObject2.getBoolean("v")));
            } catch (JSONException e2) {
                getConfigLogger().verbose(getLogTag(), "Error parsing Feature Flag array " + e2.getLocalizedMessage());
            }
        }
    ...
    String getCachedFullPath() {
        return getCachedDirName() + MqttTopic.TOPIC_LEVEL_SEPARATOR + getCachedFileName();
    }
            ...
            public Boolean call() {
                synchronized (this) {
                    ...
                    try {
                        ...
                        e2.printStackTrace();
                        CTFeatureFlagsController.this.getConfigLogger().verbose(CTFeatureFlagsController.this.getLogTag(), "UnArchiveData failed file- " + cachedFullPath + " " + e2.getLocalizedMessage());
                        return false;
                    }
                }
                return true;
