...
public class CTCarouselViewPagerAdapter extends PagerAdapter {
    ...
    public Object instantiateItem(ViewGroup viewGroup, int i) {
        ...
        try {
            if (this.inboxMessage.getOrientation().equalsIgnoreCase(CmcdData.Factory.STREAM_TYPE_LIVE)) {
    ...
    void addImageAndSetClick(ImageView imageView, View view, final int i, ViewGroup viewGroup) {
        ...
        String contentDescription = this.carouselImagesData.get(i).getContentDescription();
        if (contentDescription.isEmpty()) {
            contentDescription = this.context.getString(R.string.ct_inbox_image_content_description) + (i + 1);
        }
        imageView.setContentDescription(contentDescription);
        try {
            Glide.with(imageView.getContext()).load(this.carouselImagesData.get(i).getUrl()).apply((BaseRequestOptions<?>) new RequestOptions().placeholder(Utils.getThumbnailImage(this.context, Constants.IMAGE_PLACEHOLDER)).error(Utils.getThumbnailImage(this.context, Constants.IMAGE_PLACEHOLDER))).into(imageView);
        } catch (NoSuchMethodError unused) {
            Logger.d("CleverTap SDK requires Glide v4.9.0 or above. Please refer CleverTap Documentation for more info");
            Glide.with(imageView.getContext()).load(this.carouselImagesData.get(i).getUrl()).into(imageView);
        }
