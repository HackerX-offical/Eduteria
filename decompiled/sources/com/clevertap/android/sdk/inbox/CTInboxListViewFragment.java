...
public class CTInboxListViewFragment extends Fragment {
    ...
    public void onAttach(Context context) {
        ...
        if (arguments != null) {
            ...
            this.tabPosition = arguments.getInt(Const.POSITION, -1);
            updateInboxMessages();
    ...
    void didShow(Bundle bundle, int i) {
        InboxListener listener = getListener();
        if (listener != null) {
            Logger.v("CTInboxListViewFragment:didShow() called with: data = [" + bundle + "], position = [" + i + Constants.AES_SUFFIX);
    ...
    private ArrayList<CTInboxMessage> filterMessages(ArrayList<CTInboxMessage> arrayList, String str) {
        ...
        for (CTInboxMessage cTInboxMessage : arrayList) {
            if (cTInboxMessage.getTags() != null && cTInboxMessage.getTags().size() > 0) {
                Iterator<String> it = cTInboxMessage.getTags().iterator();
                while (it.hasNext()) {
