...
public final class ManifestValidator {
    ...
    private static void checkReceiversServices(Context context, PushProviders pushProviders) {
        try {
            ...
            Logger.v("Receiver/Service issue : " + e2.toString());
        }
        ArrayList<PushType> availablePushTypes = pushProviders.getAvailablePushTypes();
        if (availablePushTypes == null) {
            return;
        }
        Iterator<PushType> it = availablePushTypes.iterator();
        while (it.hasNext()) {
            if (it.next() == PushConstants.FCM) {
                try {
                    ...
                    Logger.v("FATAL : " + e3.getMessage());
                } catch (Exception e4) {
                    Logger.v("Receiver/Service issue : " + e4.toString());
                }
            }
        }
    }
