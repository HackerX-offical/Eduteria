...
class DragPinchManager implements GestureDetector.OnGestureListener, GestureDetector.OnDoubleTapListener, ScaleGestureDetector.OnScaleGestureListener, View.OnTouchListener {
    ...
    public boolean onSingleTapConfirmed(MotionEvent motionEvent) {
        ...
        boolean zCallOnTap = this.pdfView.callbacks.callOnTap(motionEvent);
    ...
    private boolean checkLinkTapped(float f2, float f3) {
        ...
        float f4 = (-this.pdfView.getCurrentXOffset()) + f2;
        ...
        for (PdfDocument.Link link : pdfFile.getPageLinks(pageAtOffset)) {
            RectF rectFMapRectToDevice = pdfFile.mapRectToDevice(pageAtOffset, pageOffset, secondaryPageOffset, (int) scaledPageSize.getWidth(), (int) scaledPageSize.getHeight(), link.getBounds());
            rectFMapRectToDevice.sort();
            if (rectFMapRectToDevice.contains(f4, f5)) {
                this.pdfView.callbacks.callLinkHandler(new LinkTapEvent(f2, f3, f4, f5, rectFMapRectToDevice, link));
                return true;
            }
        }
        return false;
    ...
    private void startPageFling(MotionEvent motionEvent, MotionEvent motionEvent2, float f2, float f3) {
        ...
        if (checkDoPageFling(f2, f3)) {
            ...
            float f4 = x - x2;
            int iMax = Math.max(0, Math.min(this.pdfView.getPageCount() - 1, this.pdfView.findFocusPage(this.pdfView.getCurrentXOffset() - (this.pdfView.getZoom() * f4), this.pdfView.getCurrentYOffset() - (f4 * this.pdfView.getZoom())) + i));
            this.animationManager.startPageFlingAnimation(-this.pdfView.snapOffsetForPage(iMax, this.pdfView.findSnapEdge(iMax)));
        }
    }
    ...
    private boolean checkDoPageFling(float f2, float f3) {
        ...
        return this.pdfView.isSwipeVertical() ? fAbs2 > fAbs : fAbs > fAbs2;
    }
}
