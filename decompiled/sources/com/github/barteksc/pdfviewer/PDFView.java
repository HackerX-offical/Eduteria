...
public class PDFView extends RelativeLayout {
    ...
    public void jumpTo(int i, boolean z) {
        ...
        int iDetermineValidPageNumberFrom = pdfFile.determineValidPageNumberFrom(i);
        float f2 = iDetermineValidPageNumberFrom == 0 ? 0.0f : -this.pdfFile.getPageOffset(iDetermineValidPageNumberFrom, this.zoom);
        ...
        showPage(iDetermineValidPageNumberFrom);
    }
    ...
    public void setPositionOffset(float f2, boolean z) {
        if (this.swipeVertical) {
            moveTo(this.currentXOffset, ((-this.pdfFile.getDocLen(this.zoom)) + getHeight()) * f2, z);
        } else {
            moveTo(((-this.pdfFile.getDocLen(this.zoom)) + getWidth()) * f2, this.currentYOffset, z);
        }
        loadPageByOffset();
    }
    ...
    protected void onSizeChanged(int i, int i2, int i3, int i4) {
        ...
        if (this.swipeVertical) {
            docLen = f2 / this.pdfFile.getMaxPageWidth();
            maxPageHeight = this.pdfFile.getDocLen(this.zoom);
        } else {
            docLen = f2 / this.pdfFile.getDocLen(this.zoom);
        ...
        if (this.swipeVertical) {
            ...
            this.currentYOffset = ((-f4) * this.pdfFile.getDocLen(this.zoom)) + (i2 * 0.5f);
        } else {
            this.currentXOffset = ((-docLen) * this.pdfFile.getDocLen(this.zoom)) + (i * 0.5f);
            this.currentYOffset = ((-f4) * this.pdfFile.getMaxPageHeight()) + (i2 * 0.5f);
        }
    ...
    private void drawWithListener(Canvas canvas, int i, OnDrawListener onDrawListener) {
        ...
        if (onDrawListener != null) {
            ...
            if (this.swipeVertical) {
                ...
                pageOffset2 = this.pdfFile.getPageOffset(i, this.zoom);
                pageOffset = 0.0f;
            }
    ...
    */
    ...
    public void performPageSnap() {
        PdfFile pdfFile;
        int iFindFocusPage;
        SnapEdge snapEdgeFindSnapEdge;
        if (!this.pageSnap || (pdfFile = this.pdfFile) == null || pdfFile.getPagesCount() == 0 || (snapEdgeFindSnapEdge = findSnapEdge((iFindFocusPage = findFocusPage(this.currentXOffset, this.currentYOffset)))) == SnapEdge.NONE) {
            return;
        }
        float fSnapOffsetForPage = snapOffsetForPage(iFindFocusPage, snapEdgeFindSnapEdge);
        if (this.swipeVertical) {
            this.animationManager.startYAnimation(this.currentYOffset, -fSnapOffsetForPage);
        } else {
            this.animationManager.startXAnimation(this.currentXOffset, -fSnapOffsetForPage);
        }
    }
    ...
    SnapEdge findSnapEdge(int i) {
        ...
        float f3 = -this.pdfFile.getPageOffset(i, this.zoom);
    ...
    int findFocusPage(float f2, float f3) {
        ...
        if (f2 < (-this.pdfFile.getDocLen(this.zoom)) + height + 1.0f) {
            return this.pdfFile.getPagesCount() - 1;
        }
        return this.pdfFile.getPageAtOffset(-(f2 - (height / 2.0f)), this.zoom);
    }

    public boolean pageFillsScreen() {
        float f2 = -this.pdfFile.getPageOffset(this.currentPage, this.zoom);
        float pageLength = f2 - this.pdfFile.getPageLength(this.currentPage, this.zoom);
        if (isSwipeVertical()) {
            float f3 = this.currentYOffset;
