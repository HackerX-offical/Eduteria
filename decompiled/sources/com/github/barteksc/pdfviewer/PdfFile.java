...
class PdfFile {
    ...
    public SizeF getScaledPageSize(int i, float f2) {
    ...
    private void prepareDocLen() {
        ...
        for (int i = 0; i < getPagesCount(); i++) {
            ...
            if (this.autoSpacing) {
                fFloatValue = this.pageSpacing.get(i).floatValue();
    ...
    private void preparePagesOffset() {
        ...
        for (int i = 0; i < getPagesCount(); i++) {
            ...
            if (this.autoSpacing) {
                fFloatValue2 += this.pageSpacing.get(i).floatValue() / 2.0f;
                if (i == 0) {
                ...
                fFloatValue = this.pageSpacing.get(i).floatValue() / 2.0f;
            } else {
                this.pageOffsets.add(Float.valueOf(fFloatValue2));
                fFloatValue = this.spacingPx;
            }
    ...
    public float getPageLength(int i, float f2) {
        SizeF pageSize = getPageSize(i);
        return (this.isVertical ? pageSize.getHeight() : pageSize.getWidth()) * f2;
    }

    public float getPageSpacing(int i, float f2) {
        return (this.autoSpacing ? this.pageSpacing.get(i).floatValue() : this.spacingPx) * f2;
    }

    public float getPageOffset(int i, float f2) {
        if (documentPage(i) < 0) {
            return 0.0f;
        }
        return this.pageOffsets.get(i).floatValue() * f2;
    }

    public float getSecondaryPageOffset(int i, float f2) {
        float maxPageHeight;
        float height;
        SizeF pageSize = getPageSize(i);
        if (this.isVertical) {
    ...
    public int getPageAtOffset(float f2, float f3) {
        int i = 0;
        for (int i2 = 0; i2 < getPagesCount() && (this.pageOffsets.get(i2).floatValue() * f3) - (getPageSpacing(i2, f3) / 2.0f) < f2; i2++) {
            i++;
        }
        int i3 = i - 1;
        if (i3 >= 0) {
    ...
    public boolean openPage(int i) throws PageRenderingException {
        int iDocumentPage = documentPage(i);
        if (iDocumentPage < 0) {
            return false;
        }
        synchronized (lock) {
