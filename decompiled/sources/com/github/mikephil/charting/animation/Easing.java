...
public class Easing {
    ...
    public static final EasingFunction EaseInOutCubic = new EasingFunction() { // from class: com.github.mikephil.charting.animation.Easing.7
        ...
        public float getInterpolation(float f2) {
            ...
            float f3 = f2 * 2.0f;
            if (f3 < 1.0f) {
                fPow = (float) Math.pow(f3, 3.0d);
            } else {
                fPow = ((float) Math.pow(f3 - 2.0f, 3.0d)) + 2.0f;
            }
            return fPow * 0.5f;
        }
    };
    ...
    public static final EasingFunction EaseInOutBounce = new EasingFunction() { // from class: com.github.mikephil.charting.animation.Easing.28
        ...
        public float getInterpolation(float f2) {
            if (f2 < 0.5f) {
                return Easing.EaseInBounce.getInterpolation(f2 * 2.0f) * 0.5f;
            }
            return (Easing.EaseOutBounce.getInterpolation((f2 * 2.0f) - 1.0f) * 0.5f) + 0.5f;
        }
    };
