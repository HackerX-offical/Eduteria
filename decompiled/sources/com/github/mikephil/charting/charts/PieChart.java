...
public class PieChart extends PieRadarChartBase<PieData> {
    ...
    protected float[] getMarkerPosition(Highlight highlight) {
        ...
        double d2 = f2;
        float fCos = (float) ((Math.cos(Math.toRadians(((this.mAbsoluteAngles[r11] + rotationAngle) - f3) * this.mAnimator.getPhaseY())) * d2) + ((double) centerCircleBox.x));
        float fSin = (float) ((d2 * Math.sin(Math.toRadians(((rotationAngle + this.mAbsoluteAngles[r11]) - f3) * this.mAnimator.getPhaseY()))) + ((double) centerCircleBox.y));
    ...
    private void calcAngles() {
        ...
        boolean z = f4 != 0.0f && ((float) entryCount) * f4 <= this.mMaxAngle;
        ...
        if (z) {
            for (int i6 = 0; i6 < entryCount; i6++) {
                float f9 = fArr[i6];
                float f10 = f9 - (((f9 - this.mMinAngleForSlices) / f6) * f5);
                fArr[i6] = f10;
                if (i6 == 0) {
                    this.mAbsoluteAngles[0] = fArr[0];
                } else {
                    float[] fArr3 = this.mAbsoluteAngles;
                    fArr3[i6] = fArr3[i6 - 1] + f10;
                }
            }
