...
public class YAxisRendererHorizontalBarChart extends YAxisRenderer {
    ...
    protected void drawYLabels(Canvas canvas, float f2, float[] fArr, float f3) {
        ...
        for (int i2 = !this.mYAxis.isDrawBottomYLabelEntryEnabled() ? 1 : 0; i2 < i; i2++) {
            canvas.drawText(this.mYAxis.getFormattedLabel(i2), fArr[i2 * 2], f2 - f3, this.mAxisLabelPaint);
        }
    }
            ...
            if (limitLine.isEnabled()) {
                ...
                if (label == null || label.equals("")) {
                    ...
                    if (labelPosition == LimitLine.LimitLabelPosition.RIGHT_TOP) {
                        ...
                        this.mLimitLinePaint.setTextAlign(Paint.Align.LEFT);
                        canvas.drawText(label, fArr[c2] + lineWidth, this.mViewPortHandler.contentTop() + fConvertDpToPixel + fCalcTextHeight, this.mLimitLinePaint);
                    } else if (labelPosition == LimitLine.LimitLabelPosition.RIGHT_BOTTOM) {
                        this.mLimitLinePaint.setTextAlign(Paint.Align.LEFT);
                        canvas.drawText(label, fArr[c2] + lineWidth, this.mViewPortHandler.contentBottom() - fConvertDpToPixel, this.mLimitLinePaint);
                    } else if (labelPosition == LimitLine.LimitLabelPosition.LEFT_TOP) {
                        this.mLimitLinePaint.setTextAlign(Paint.Align.RIGHT);
                        canvas.drawText(label, fArr[c2] - lineWidth, this.mViewPortHandler.contentTop() + fConvertDpToPixel + Utils.calcTextHeight(this.mLimitLinePaint, label), this.mLimitLinePaint);
                    } else {
                        this.mLimitLinePaint.setTextAlign(Paint.Align.RIGHT);
                        canvas.drawText(label, fArr[c2] - lineWidth, this.mViewPortHandler.contentBottom() - fConvertDpToPixel, this.mLimitLinePaint);
                    }
