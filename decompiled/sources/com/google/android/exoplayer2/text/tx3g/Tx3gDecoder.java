...
public final class Tx3gDecoder extends SimpleSubtitleDecoder {
    ...
    private void applyStyleRecord(ParsableByteArray parsableByteArray, SpannableStringBuilder spannableStringBuilder) throws SubtitleDecoderException {
        ...
        if (unsignedShort2 > spannableStringBuilder.length()) {
            Log.w(TAG, new StringBuilder(68).append("Truncating styl end (").append(unsignedShort2).append(") to cueText.length() (").append(spannableStringBuilder.length()).append(").").toString());
            unsignedShort2 = spannableStringBuilder.length();
        }
        int i2 = unsignedShort2;
        if (unsignedShort >= i2) {
            Log.w(TAG, new StringBuilder(60).append("Ignoring styl with start (").append(unsignedShort).append(") >= end (").append(i2).append(").").toString());
        } else {
