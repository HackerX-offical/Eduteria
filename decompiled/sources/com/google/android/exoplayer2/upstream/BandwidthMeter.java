...
public interface BandwidthMeter {
    ...
    public interface EventListener {
        ...
        public static final class EventDispatcher {
            ...
            private final CopyOnWriteArrayList<HandlerAndListener> f603listeners = new CopyOnWriteArrayList<>();

            public void addListener(Handler handler, EventListener eventListener) {
                Assertions.checkNotNull(handler);
                Assertions.checkNotNull(eventListener);
                removeListener(eventListener);
                this.f603listeners.add(new HandlerAndListener(handler, eventListener));
            }

            public void removeListener(EventListener eventListener) {
                for (HandlerAndListener handlerAndListener : this.f603listeners) {
                    if (handlerAndListener.listener == eventListener) {
                        handlerAndListener.release();
                        this.f603listeners.remove(handlerAndListener);
                    }
                }
            }
            ...
            public void bandwidthSample(int i, long j, long j2) {
                ...
                final long j3;
                final long j4;
                for (final HandlerAndListener handlerAndListener : this.f603listeners) {
