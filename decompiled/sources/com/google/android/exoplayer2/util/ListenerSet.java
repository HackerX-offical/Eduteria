...
public final class ListenerSet<T> {
    ...
    private final CopyOnWriteArraySet<ListenerHolder<T>> f606listeners;
    ...
    private ListenerSet(CopyOnWriteArraySet<ListenerHolder<T>> copyOnWriteArraySet, Looper looper, Clock clock, IterationFinishedEvent<T> iterationFinishedEvent) {
        this.clock = clock;
        this.f606listeners = copyOnWriteArraySet;
    ...
    public ListenerSet<T> copy(Looper looper, Clock clock, IterationFinishedEvent<T> iterationFinishedEvent) {
        return new ListenerSet<>(this.f606listeners, looper, clock, iterationFinishedEvent);
    }
    ...
    public void add(T t) {
        ...
        this.f606listeners.add(new ListenerHolder<>(t));
    }

    public void remove(T t) {
        for (ListenerHolder<T> listenerHolder : this.f606listeners) {
            if (listenerHolder.listener.equals(t)) {
                listenerHolder.release(this.iterationFinishedEvent);
                this.f606listeners.remove(listenerHolder);
            }
        }
    }

    public void clear() {
        this.f606listeners.clear();
    }

    public int size() {
        return this.f606listeners.size();
    }

    public void queueEvent(final int i, final Event<T> event) {
        final CopyOnWriteArraySet copyOnWriteArraySet = new CopyOnWriteArraySet(this.f606listeners);
    ...
    public void release() {
        Iterator<ListenerHolder<T>> it = this.f606listeners.iterator();
        while (it.hasNext()) {
