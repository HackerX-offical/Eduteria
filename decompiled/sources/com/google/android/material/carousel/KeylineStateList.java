...
class KeylineStateList {
    ...
    private static KeylineState lerp(List<KeylineState> list, float f2, float[] fArr) {
        ...
        while (i < size) {
            float f4 = fArr[i];
            if (f2 <= f4) {
                return KeylineState.lerp(list.get(i - 1), list.get(i), AnimationUtils.lerp(0.0f, 1.0f, f3, f4, f2));
            }
            i++;
            f3 = f4;
        }
    ...
    private static float[] getStateStepInterpolationPoints(float f2, List<KeylineState> list, boolean z) {
        ...
        while (i < size) {
            ...
            fArr[i] = i == size + (-1) ? 1.0f : fArr[i2] + (f3 / f2);
            i++;
        }
        return fArr;
    }
    ...
    private static List<KeylineState> getStateStepsLeft(KeylineState keylineState) {
        ...
        if (!isFirstFocalItemAtLeftOfContainer(keylineState) && iFindFirstInBoundsKeylineIndex != -1) {
            ...
            for (int i = 0; i <= firstFocalKeylineIndex; i++) {
                ...
                int i2 = (iFindFirstInBoundsKeylineIndex + i) - 1;
                if (i2 >= 0) {
                    size = findFirstIndexAfterLastFocalKeylineWithMask(keylineState2, keylineState.getKeylines().get(i2).mask) - 1;
                }
                arrayList.add(moveKeylineAndCreateKeylineState(keylineState2, iFindFirstInBoundsKeylineIndex, size, f2, (keylineState.getFirstFocalKeylineIndex() - i) - 1, (keylineState.getLastFocalKeylineIndex() - i) - 1));
            }
        }
        return arrayList;
    }
    ...
    private static List<KeylineState> getStateStepsRight(Carousel carousel, KeylineState keylineState) {
        ...
        if (!isLastFocalItemAtRightOfContainer(carousel, keylineState) && iFindLastInBoundsKeylineIndex != -1) {
            ...
            for (int i = 0; i < lastFocalKeylineIndex; i++) {
                KeylineState keylineState2 = (KeylineState) arrayList.get(arrayList.size() - 1);
                int i2 = (iFindLastInBoundsKeylineIndex - i) + 1;
                arrayList.add(moveKeylineAndCreateKeylineState(keylineState2, iFindLastInBoundsKeylineIndex, i2 < keylineState.getKeylines().size() ? findLastIndexBeforeFirstFocalKeylineWithMask(keylineState2, keylineState.getKeylines().get(i2).mask) + 1 : 0, f2, keylineState.getFirstFocalKeylineIndex() + i + 1, keylineState.getLastFocalKeylineIndex() + i + 1));
            }
        }
        return arrayList;
    }

    private static KeylineState moveKeylineAndCreateKeylineState(KeylineState keylineState, int i, int i2, float f2, int i3, int i4) {
        ...
        while (i5 < arrayList.size()) {
            ...
            builder.addKeyline((keyline.maskedItemSize / 2.0f) + f2, keyline.mask, keyline.maskedItemSize, i5 >= i3 && i5 <= i4);
