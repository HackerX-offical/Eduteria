...
public class HctSolver {
    ...
    static double intercept(double d2, double d3, double d4) {
        return (d3 - d2) / (d4 - d2);
    }

    static boolean isBounded(double d2) {
        return 0.0d <= d2 && d2 <= 100.0d;
    }

    static double sanitizeRadians(double d2) {
    ...
    static double trueDelinearized(double d2) {
        double d3 = d2 / 100.0d;
        return (d3 <= 0.0031308d ? d3 * 12.92d : (Math.pow(d3, 0.4166666666666667d) * 1.055d) - 0.055d) * 255.0d;
    }

    static double chromaticAdaptation(double d2) {
        double dPow = Math.pow(Math.abs(d2), 0.42d);
        return ((((double) MathUtils.signum(d2)) * 400.0d) * dPow) / (dPow + 27.13d);
    }
    ...
    static boolean areInCyclicOrder(double d2, double d3, double d4) {
        return sanitizeRadians(d3 - d2) < sanitizeRadians(d4 - d2);
    }

    static double[] lerpPoint(double[] dArr, double d2, double[] dArr2) {
        double d3 = dArr[0];
        double d4 = d3 + ((dArr2[0] - d3) * d2);
        double d5 = dArr[1];
        double d6 = d5 + ((dArr2[1] - d5) * d2);
        double d7 = dArr[2];
        return new double[]{d4, d6, d7 + ((dArr2[2] - d7) * d2)};
    }

    static double[] setCoordinate(double[] dArr, double d2, double[] dArr2, int i) {
        return lerpPoint(dArr, intercept(dArr[i], d2, dArr2[i]), dArr2);
    }

    static double[] nthVertex(double d2, int i) {
        double[] dArr = Y_FROM_LINRGB;
        double d3 = dArr[0];
        double d4 = dArr[1];
        double d5 = dArr[2];
        double d6 = i % 4 <= 1 ? 0.0d : 100.0d;
        double d7 = i % 2 != 0 ? 100.0d : 0.0d;
        if (i < 4) {
            double d8 = ((d2 - (d4 * d6)) - (d5 * d7)) / d3;
            if (isBounded(d8)) {
                return new double[]{d8, d6, d7};
            }
        ...
        if (i < 8) {
            ...
            if (isBounded(d9)) {
                return new double[]{d7, d9, d6};
            }
            return new double[]{-1.0d, -1.0d, -1.0d};
        }
        double d10 = ((d2 - (d3 * d6)) - (d4 * d7)) / d5;
        if (isBounded(d10)) {
            return new double[]{d6, d7, d10};
        }
        return new double[]{-1.0d, -1.0d, -1.0d};
    }

    static double[][] bisectToSegment(double d2, double d3) {
        double d4;
        ...
        boolean z2 = true;
        for (int i = 0; i < 12; i++) {
            double[] dArrNthVertex = nthVertex(d2, i);
            if (dArrNthVertex[0] < 0.0d) {
                ...
                if (z) {
                    if (z2) {
                        ...
                        boolean zAreInCyclicOrder = areInCyclicOrder(d5, dHueOf, d6);
                        d4 = d6;
                        d6 = dHueOf;
                        if (zAreInCyclicOrder) {
                        }
                    }
                    if (areInCyclicOrder(d5, d3, d6)) {
                        z2 = false;
                        dArr2 = dArrNthVertex;
                    } else {
                        z2 = false;
    ...
    static int criticalPlaneBelow(double d2) {
        return (int) Math.floor(d2 - 0.5d);
    }

    static int criticalPlaneAbove(double d2) {
        return (int) Math.ceil(d2 - 0.5d);
    }

    static double[] bisectToLimit(double d2, double d3) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (d4 != d5) {
                ...
                for (int i4 = 0; i4 < 8 && Math.abs(i3 - i2) > 1; i4++) {
                    ...
                    if (areInCyclicOrder(d7, d3, dHueOf2)) {
                        i3 = iFloor;
                        dArr2 = coordinate;
                    } else {
                        d7 = dHueOf2;
    ...
    static double inverseChromaticAdaptation(double d2) {
        double dAbs = Math.abs(d2);
        return ((double) MathUtils.signum(d2)) * Math.pow(Math.max(0.0d, (27.13d * dAbs) / (400.0d - dAbs)), 2.380952380952381d);
    }

    static int findResultByJ(double d2, double d3, double d4) {
        double dSqrt = Math.sqrt(d4) * 11.0d;
        ViewingConditions viewingConditions = ViewingConditions.DEFAULT;
        double d5 = 1.0d;
        double dPow = 1.0d / Math.pow(1.64d - Math.pow(0.29d, viewingConditions.getN()), 0.73d);
        double d6 = 2.0d;
        double dCos = (Math.cos(d2 + 2.0d) + 3.8d) * 0.25d * 3846.153846153846d * viewingConditions.getNc() * viewingConditions.getNcb();
        ...
        while (i < 5) {
            ...
            double d9 = d6;
            double d10 = dSqrt;
            double dPow2 = Math.pow(((d3 == 0.0d || dSqrt == 0.0d) ? 0.0d : d3 / Math.sqrt(d8)) * dPow, 1.1111111111111112d);
            double aw = (viewingConditions.getAw() * Math.pow(d8, (d7 / viewingConditions.getC()) / viewingConditions.getZ())) / viewingConditions.getNbb();
            double d11 = (((0.305d + aw) * 23.0d) * dPow2) / (((23.0d * dCos) + ((dPow2 * 11.0d) * dCos2)) + ((108.0d * dPow2) * dSin));
            double d12 = d11 * dCos2;
            double d13 = d11 * dSin;
            double d14 = aw * 460.0d;
            double[] dArrMatrixMultiply = MathUtils.matrixMultiply(new double[]{inverseChromaticAdaptation(((d14 + (451.0d * d12)) + (288.0d * d13)) / 1403.0d), inverseChromaticAdaptation(((d14 - (891.0d * d12)) - (261.0d * d13)) / 1403.0d), inverseChromaticAdaptation(((d14 - (d12 * 220.0d)) - (d13 * 6300.0d)) / 1403.0d)}, LINRGB_FROM_SCALED_DISCOUNT);
            ...
            double[] dArr = Y_FROM_LINRGB;
            double d18 = (dArr[0] * d15) + (dArr[1] * d16) + (dArr[2] * d17);
            if (d18 <= 0.0d) {
                return 0;
            }
            if (i != 4) {
