...
public final class CollapsingTextHelper {
    ...
    private float calculateFadeModeTextAlpha(float f2) {
        ...
        if (f2 <= f3) {
            return AnimationUtils.lerp(1.0f, 0.0f, this.fadeModeStartFraction, f3, f2);
        }
        return AnimationUtils.lerp(0.0f, 1.0f, f3, 1.0f, f2);
    }
    ...
    private void calculateUsingTextSize(float f2, boolean z) {
        ...
        if (isClose(f2, 1.0f)) {
            ...
            fWidth = (!z && fWidth2 * f7 > fWidth) ? Math.min(fWidth / f7, fWidth2) : fWidth2;
            f3 = f5;
            f4 = f6;
            typeface = typeface2;
        }
        if (fWidth > 0.0f) {
            boolean z2 = this.currentTextSize != f3;
            boolean z3 = this.currentLetterSpacing != f4;
    ...
    private StaticLayout createStaticLayout(int i, float f2, boolean z) {
        ...
        try {
            staticLayoutBuild = StaticLayoutBuilderCompat.obtain(this.text, this.textPaint, (int) f2).setEllipsize(this.titleTextEllipsize).setIsRtl(z).setAlignment(i == 1 ? Layout.Alignment.ALIGN_NORMAL : getMultilineTextLayoutAlignment()).setIncludePad(false).setMaxLines(i).setLineSpacing(this.lineSpacingAdd, this.lineSpacingMultiplier).setHyphenationFrequency(this.hyphenationFrequency).setStaticLayoutBuilderConfigurer(this.staticLayoutBuilderConfigurer).build();
    ...
    private static int blendARGB(int i, int i2, float f2) {
        float f3 = 1.0f - f2;
