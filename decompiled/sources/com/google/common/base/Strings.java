...
public final class Strings {
    ...
    public static String commonPrefix(CharSequence a2, CharSequence b2) {
        ...
        int i = 0;
        while (i < iMin && a2.charAt(i) == b2.charAt(i)) {
            i++;
        }
        int i2 = i - 1;
        if (validSurrogatePairAt(a2, i2) || validSurrogatePairAt(b2, i2)) {
            i--;
        }
    ...
    public static String commonSuffix(CharSequence a2, CharSequence b2) {
        ...
        int iMin = Math.min(a2.length(), b2.length());
        int i = 0;
        while (i < iMin && a2.charAt((a2.length() - i) - 1) == b2.charAt((b2.length() - i) - 1)) {
            i++;
        }
        if (validSurrogatePairAt(a2, (a2.length() - i) - 1) || validSurrogatePairAt(b2, (b2.length() - i) - 1)) {
            i--;
        }
