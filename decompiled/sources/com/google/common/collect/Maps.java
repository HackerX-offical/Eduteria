...
public final class Maps {
    ...
    private static <K, V> void doDifference(Map<? extends K, ? extends V> map, Map<? extends K, ? extends V> map2, Equivalence<? super V> equivalence, Map<K, V> map3, Map<K, V> map4, Map<K, V> map5, Map<K, MapDifference.ValueDifference<V>> map6) {
        for (Map.Entry<? extends K, ? extends V> entry : map.entrySet()) {
            ...
            if (map2.containsKey(key)) {
                PdfBookMark pdfBookMark = (Object) NullnessCasts.uncheckedCastNullableTToT(map4.remove(key));
                if (equivalence.equivalent(value, pdfBookMark)) {
                    map5.put(key, value);
                } else {
                    map6.put(key, ValueDifferenceImpl.create(value, pdfBookMark));
                }
            } else {
                map3.put(key, value);
            }
    ...
    static <K, V> boolean containsEntryImpl(Collection<Map.Entry<K, V>> c2, @CheckForNull Object o) {
        if (o instanceof Map.Entry) {
            return c2.contains(unmodifiableEntry((Map.Entry) o));
        }
        return false;
    }

    static <K, V> boolean removeEntryImpl(Collection<Map.Entry<K, V>> c2, @CheckForNull Object o) {
        if (o instanceof Map.Entry) {
            return c2.remove(unmodifiableEntry((Map.Entry) o));
        }
        return false;
    }
    ...
    static class NavigableKeySet<K, V> extends SortedKeySet<K, V> implements NavigableSet<K> {
        ...
        @CheckForNull
        public K lower(@ParametricNullness K e2) {
            return map().lowerKey(e2);
        }

        @Override // java.util.NavigableSet
        @CheckForNull
        public K floor(@ParametricNullness K e2) {
            return map().floorKey(e2);
        }

        @Override // java.util.NavigableSet
        @CheckForNull
        public K ceiling(@ParametricNullness K e2) {
            return map().ceilingKey(e2);
        }

        @Override // java.util.NavigableSet
        @CheckForNull
        public K higher(@ParametricNullness K e2) {
            return map().higherKey(e2);
        }
    ...
    static class Values<K, V> extends AbstractCollection<V> {
        ...
        public boolean removeAll(Collection<?> c2) {
            try {
                return super.removeAll((Collection) Preconditions.checkNotNull(c2));
            } catch (UnsupportedOperationException unused) {
                HashSet hashSetNewHashSet = Sets.newHashSet();
                for (Map.Entry<K, V> entry : map().entrySet()) {
                    if (c2.contains(entry.getValue())) {
    ...
    static abstract class EntrySet<K, V> extends Sets.ImprovedAbstractSet<Map.Entry<K, V>> {
        ...
        public boolean retainAll(Collection<?> c2) {
            try {
                ...
                HashSet hashSetNewHashSetWithExpectedSize = Sets.newHashSetWithExpectedSize(c2.size());
                for (Object obj : c2) {
