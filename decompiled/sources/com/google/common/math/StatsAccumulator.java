...
public final class StatsAccumulator {
    ...
    public void add(double value) {
        ...
        if (Doubles.isFinite(value) && Doubles.isFinite(this.mean)) {
            ...
            this.mean = d4;
            this.sumOfSquaresOfDeltas += d3 * (value - d4);
        } else {
    ...
    private void merge(long otherCount, double otherMean, double otherSumOfSquaresOfDeltas, double otherMin, double otherMax) {
        ...
        if (Doubles.isFinite(this.mean) && Doubles.isFinite(otherMean)) {
            double d2 = this.mean;
            double d3 = otherMean - d2;
            double d4 = otherCount;
            double d5 = d2 + ((d3 * d4) / this.count);
            this.mean = d5;
            this.sumOfSquaresOfDeltas += otherSumOfSquaresOfDeltas + (d3 * (otherMean - d5) * d4);
        } else {
