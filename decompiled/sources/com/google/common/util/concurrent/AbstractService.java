...
public abstract class AbstractService implements Service {
    ...
    private final ListenerCallQueue<Service.Listener> f636listeners = new ListenerCallQueue<>();
    ...
    public final void addListener(Service.Listener listener, Executor executor) {
        this.f636listeners.addListener(listener, executor);
    }

    public String toString() {
        return getClass().getSimpleName() + " [" + state() + Constants.AES_SUFFIX;
    }
    ...
    private void dispatchListenerEvents() throws Exception {
        ...
        this.f636listeners.dispatch();
    }

    private void enqueueStartingEvent() {
        this.f636listeners.enqueue(STARTING_EVENT);
    }

    private void enqueueRunningEvent() {
        this.f636listeners.enqueue(RUNNING_EVENT);
    }

    private void enqueueStoppingEvent(final Service.State from) {
        if (from == Service.State.STARTING) {
            this.f636listeners.enqueue(STOPPING_FROM_STARTING_EVENT);
        } else {
            if (from == Service.State.RUNNING) {
                this.f636listeners.enqueue(STOPPING_FROM_RUNNING_EVENT);
                return;
            }
    ...
    private void enqueueTerminatedEvent(final Service.State from) {
        switch (AnonymousClass6.$SwitchMap$com$google$common$util$concurrent$Service$State[from.ordinal()]) {
            case 1:
                this.f636listeners.enqueue(TERMINATED_FROM_NEW_EVENT);
                return;
            case 2:
                this.f636listeners.enqueue(TERMINATED_FROM_STARTING_EVENT);
                return;
            case 3:
                this.f636listeners.enqueue(TERMINATED_FROM_RUNNING_EVENT);
                return;
            case 4:
                this.f636listeners.enqueue(TERMINATED_FROM_STOPPING_EVENT);
                return;
            case 5:
            case 6:
                throw new AssertionError();
    ...
    private void enqueueFailedEvent(final Service.State from, final Throwable cause) {
        this.f636listeners.enqueue(new ListenerCallQueue.Event<Service.Listener>(this) { // from class: com.google.common.util.concurrent.AbstractService.5
