...
final class ListenerCallQueue<L> {
    ...
    private final List<PerListenerQueue<L>> f637listeners = Collections.synchronizedList(new ArrayList());
    ...
    public void addListener(L listener, Executor executor) {
        ...
        this.f637listeners.add(new PerListenerQueue<>(listener, executor));
    }
    ...
    private void enqueueHelper(Event<L> event, Object label) {
        ...
        synchronized (this.f637listeners) {
            Iterator<PerListenerQueue<L>> it = this.f637listeners.iterator();
            while (it.hasNext()) {
    ...
    public void dispatch() throws Exception {
        for (int i = 0; i < this.f637listeners.size(); i++) {
            this.f637listeners.get(i).dispatch();
        }
    }
            ...
            if (z) {
                try {
                    ...
                    synchronized (this) {
                        ...
                        ListenerCallQueue.logger.get().log(Level.SEVERE, "Exception while running callbacks for " + this.listener + " on " + this.executor, (Throwable) e2);
                        throw e2;
                    }
                }
            }
