...
final class ChaCha20Util {
    ...
    static void quarterRound(int[] x, int a2, int b2, int c2, int d2) {
        int i = x[a2] + x[b2];
        x[a2] = i;
        int iRotateLeft = rotateLeft(i ^ x[d2], 16);
        x[d2] = iRotateLeft;
        int i2 = x[c2] + iRotateLeft;
        x[c2] = i2;
        int iRotateLeft2 = rotateLeft(x[b2] ^ i2, 12);
        x[b2] = iRotateLeft2;
        int i3 = x[a2] + iRotateLeft2;
        x[a2] = i3;
        int iRotateLeft3 = rotateLeft(x[d2] ^ i3, 8);
        x[d2] = iRotateLeft3;
        int i4 = x[c2] + iRotateLeft3;
        x[c2] = i4;
        x[b2] = rotateLeft(x[b2] ^ i4, 7);
    }
