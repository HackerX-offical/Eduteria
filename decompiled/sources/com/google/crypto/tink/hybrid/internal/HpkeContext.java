...
final class HpkeContext {
    ...
    static HpkeContext createContext(byte[] encapsulatedKey, byte[] sharedSecret, HpkeKem kem, HpkeKdf kdf, HpkeAead aead, byte[] info2) throws GeneralSecurityException {
        ...
        byte[] bArrConcat = Bytes.concat(HpkeUtil.BASE_MODE, kdf.labeledExtract(bArr, bArr2, "psk_id_hash", bArrHpkeSuiteId), kdf.labeledExtract(HpkeUtil.EMPTY_SALT, info2, "info_hash", bArrHpkeSuiteId));
    ...
    static HpkeContext createSenderContext(HpkePublicKey recipientPublicKey, HpkeKem kem, HpkeKdf kdf, HpkeAead aead, byte[] info2) throws GeneralSecurityException {
        HpkeKemEncapOutput hpkeKemEncapOutputEncapsulate = kem.encapsulate(recipientPublicKey.getPublicKey().toByteArray());
        return createContext(hpkeKemEncapOutputEncapsulate.getEncapsulatedKey(), hpkeKemEncapOutputEncapsulate.getSharedSecret(), kem, kdf, aead, info2);
    }

    static HpkeContext createRecipientContext(byte[] encapsulatedKey, HpkeKemPrivateKey recipientPrivateKey, HpkeKem kem, HpkeKdf kdf, HpkeAead aead, byte[] info2) throws GeneralSecurityException {
        return createContext(encapsulatedKey, kem.decapsulate(encapsulatedKey, recipientPrivateKey), kem, kdf, aead, info2);
    }
