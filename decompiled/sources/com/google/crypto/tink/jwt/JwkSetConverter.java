...
public final class JwkSetConverter {
    ...
    private static JsonObject convertJwtRsaSsaPkcs1(ProtoKeySerialization protoKeySerialization) throws GeneralSecurityException {
        ...
        try {
            ...
            jsonObject.addProperty(CTProductConfigConstants.PRODUCT_CONFIG_JSON_KEY_FOR_KEY, Base64.urlSafeEncode(from.getN().toByteArray()));
    ...
    private static ProtoKeySerialization convertToRsaSsaPkcs1Key(JsonObject jsonKey) throws GeneralSecurityException {
        ...
        if (jsonKey.has("p") || jsonKey.has(XHTMLText.Q) || jsonKey.has("dp") || jsonKey.has("dq") || jsonKey.has("d") || jsonKey.has("qi")) {
        ...
        JwtRsaSsaPkcs1PublicKey.Builder n = JwtRsaSsaPkcs1PublicKey.newBuilder().setVersion(0).setAlgorithm(jwtRsaSsaPkcs1Algorithm).setE(ByteString.copyFrom(Base64.urlSafeDecode(getStringItem(jsonKey, "e")))).setN(ByteString.copyFrom(Base64.urlSafeDecode(getStringItem(jsonKey, CTProductConfigConstants.PRODUCT_CONFIG_JSON_KEY_FOR_KEY))));
        if (jsonKey.has("kid")) {
    ...
    private static ProtoKeySerialization convertToRsaSsaPssKey(JsonObject jsonKey) throws GeneralSecurityException {
        ...
        if (jsonKey.has("p") || jsonKey.has(XHTMLText.Q) || jsonKey.has("dq") || jsonKey.has("dq") || jsonKey.has("d") || jsonKey.has("qi")) {
            throw new UnsupportedOperationException("importing RSA private keys is not implemented");
        }
        expectStringItem(jsonKey, "kty", "RSA");
        validateUseIsSig(jsonKey);
        validateKeyOpsIsVerify(jsonKey);
        JwtRsaSsaPssPublicKey.Builder n = JwtRsaSsaPssPublicKey.newBuilder().setVersion(0).setAlgorithm(jwtRsaSsaPssAlgorithm).setE(ByteString.copyFrom(Base64.urlSafeDecode(getStringItem(jsonKey, "e")))).setN(ByteString.copyFrom(Base64.urlSafeDecode(getStringItem(jsonKey, CTProductConfigConstants.PRODUCT_CONFIG_JSON_KEY_FOR_KEY))));
        if (jsonKey.has("kid")) {
