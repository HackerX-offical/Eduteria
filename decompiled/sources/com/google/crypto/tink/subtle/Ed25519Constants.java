...
final class Ed25519Constants {
    ...
    private static Point edwards(Point a2, Point b2) {
        Point point = new Point();
        BigInteger bigIntegerMultiply = D_BI.multiply(a2.x.multiply(b2.x).multiply(a2.y).multiply(b2.y));
        BigInteger bigInteger = P_BI;
        BigInteger bigIntegerMod = bigIntegerMultiply.mod(bigInteger);
        point.x = a2.x.multiply(b2.y).add(b2.x.multiply(a2.y)).multiply(BigInteger.ONE.add(bigIntegerMod).modInverse(bigInteger)).mod(bigInteger);
        point.y = a2.y.multiply(b2.y).add(a2.x.multiply(b2.x)).multiply(BigInteger.ONE.subtract(bigIntegerMod).modInverse(bigInteger)).mod(bigInteger);
        return point;
    }
