...
public class CrashlyticsReportPersistence {
    ...
    private static final Comparator<? super File> LATEST_SESSION_ID_FIRST_COMPARATOR = CrashlyticsReportPersistence$$Lambda$5.instance;
    private static final FilenameFilter EVENT_FILE_FILTER = CrashlyticsReportPersistence$$Lambda$6.instance;

    static /* synthetic */ int lambda$static$0(File file, File file2) {
        return file2.getName().compareTo(file.getName());
    }

    static /* synthetic */ boolean lambda$static$1(File file, String str) {
    ...
    public void persistReport(CrashlyticsReport crashlyticsReport) {
        ...
        try {
            writeTextFile(new File(prepareDirectory(getSessionDirectoryById(identifier)), REPORT_FILE_NAME), TRANSFORM.reportToJson(crashlyticsReport));
    ...
    public void deleteFinalizedReport(String str) {
        FilenameFilter filenameFilterLambdaFactory$ = CrashlyticsReportPersistence$$Lambda$1.lambdaFactory$(str);
        Iterator<File> it = combineReportFiles(getFilesInDirectory(this.priorityReportsDirectory, filenameFilterLambdaFactory$), getFilesInDirectory(this.nativeReportsDirectory, filenameFilterLambdaFactory$), getFilesInDirectory(this.reportsDirectory, filenameFilterLambdaFactory$)).iterator();
        while (it.hasNext()) {
            it.next().delete();
        }
    }
    ...
    private void synthesizeReport(File file, long j) {
        ...
        loop0: while (true) {
            ...
            for (File file2 : filesInDirectory) {
                try {
                    ...
                    Logger.getLogger().w("Could not add event to report for " + file2, e2);
                }
        ...
        if (file3.isFile()) {
            try {
                ...
                Logger.getLogger().w("Could not read user ID file in " + file.getName(), e3);
                textFile = null;
            }
        } else {
            textFile = null;
    ...
    private static int trimEvents(File file, int i) {
        List<File> filesInDirectory = getFilesInDirectory(file, CrashlyticsReportPersistence$$Lambda$3.instance);
        Collections.sort(filesInDirectory, CrashlyticsReportPersistence$$Lambda$4.instance);
