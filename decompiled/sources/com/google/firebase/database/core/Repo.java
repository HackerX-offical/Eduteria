...
public class Repo implements PersistentConnection.Delegate {
    ...
    private void restoreWrites(PersistenceManager persistenceManager) {
        ...
        for (final UserWriteRecord userWriteRecord : listLoadUserWrites) {
            ...
            if (userWriteRecord.isOverwrite()) {
                ...
                this.serverSyncTree.applyUserOverwrite(userWriteRecord.getPath(), userWriteRecord.getOverwrite(), ServerValues.resolveDeferredValueSnapshot(userWriteRecord.getOverwrite(), mapGenerateServerValues), userWriteRecord.getWriteId(), true, false);
            } else {
                if (this.operationLogger.logsDebug()) {
                ...
                this.serverSyncTree.applyUserMerge(userWriteRecord.getPath(), userWriteRecord.getMerge(), ServerValues.resolveDeferredValueMerge(userWriteRecord.getMerge(), mapGenerateServerValues), userWriteRecord.getWriteId(), false);
            }
        }
    }
    ...
    public void setValue(final Path path, Node node, final DatabaseReference.CompletionListener completionListener) {
        ...
        Node nodeResolveDeferredValueSnapshot = ServerValues.resolveDeferredValueSnapshot(node, ServerValues.generateServerValues(this.serverClock));
    ...
    public void updateChildren(final Path path, CompoundWrite compoundWrite, final DatabaseReference.CompletionListener completionListener, Map<String, Object> map) {
        ...
        CompoundWrite compoundWriteResolveDeferredValueMerge = ServerValues.resolveDeferredValueMerge(compoundWrite, ServerValues.generateServerValues(this.serverClock));
        final long nextWriteId = getNextWriteId();
    ...
    private void runOnDisconnectEvents() {
        SparseSnapshotTree sparseSnapshotTreeResolveDeferredValueTree = ServerValues.resolveDeferredValueTree(this.onDisconnect, ServerValues.generateServerValues(this.serverClock));
        final ArrayList arrayList = new ArrayList();
        sparseSnapshotTreeResolveDeferredValueTree.forEachTree(Path.getEmptyPath(), new SparseSnapshotTree.SparseSnapshotTreeVisitor() { // from class: com.google.firebase.database.core.Repo.12
    ...
    public void startTransaction(Path path, final Transaction.Handler handler, boolean z) {
        ...
        Node nodeResolveDeferredValueSnapshot = ServerValues.resolveDeferredValueSnapshot(node, mapGenerateServerValues);
            ...
            public void onRequestResult(String str, String str2) {
                ...
                if (databaseErrorFromErrorCode == null) {
                    ...
                    for (final TransactionData transactionData2 : list) {
                        ...
                        final DataSnapshot dataSnapshotCreateDataSnapshot = InternalHelpers.createDataSnapshot(InternalHelpers.createReference(this, transactionData2.path), IndexedNode.from(transactionData2.currentOutputSnapshotResolved));
                        arrayList3.add(new Runnable() { // from class: com.google.firebase.database.core.Repo.16.1
                            @Override // java.lang.Runnable
                            public void run() {
                                transactionData2.handler.onComplete(null, true, dataSnapshotCreateDataSnapshot);
                            }
                        });
                ...
                if (databaseErrorFromErrorCode.getCode() == -1) {
                    ...
                    for (TransactionData transactionData4 : list) {
                        ...
                        transactionData4.abortReason = databaseErrorFromErrorCode;
                    }
                }
    ...
    */
    ...
    public Path abortTransactions(Path path, final int i) {
        ...
        Tree<List<TransactionData>> treeSubTree = this.transactionQueueTree.subTree(path);
        treeSubTree.forEachAncestor(new Tree.TreeFilter<List<TransactionData>>() { // from class: com.google.firebase.database.core.Repo.21
        ...
        treeSubTree.forEachDescendant(new Tree.TreeVisitor<List<TransactionData>>() { // from class: com.google.firebase.database.core.Repo.22
            @Override // com.google.firebase.database.core.utilities.Tree.TreeVisitor
