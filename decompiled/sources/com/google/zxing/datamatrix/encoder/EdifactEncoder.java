...
final class EdifactEncoder implements Encoder {
    ...
    private static String encodeToCodewords(CharSequence charSequence, int i) {
        ...
        int iCharAt = (charSequence.charAt(i) << 18) + ((length >= 2 ? charSequence.charAt(i + 1) : (char) 0) << CsvReader.Letters.FORM_FEED) + ((length >= 3 ? charSequence.charAt(i + 2) : (char) 0) << 6) + (length >= 4 ? charSequence.charAt(i + 3) : (char) 0);
