...
public class BallGridPulseIndicator extends Indicator {
    ...
    public void draw(Canvas canvas, Paint paint) {
        ...
        for (int i = 0; i < 3; i++) {
            for (int i2 = 0; i2 < 3; i2++) {
                canvas.save();
                float f4 = i2;
                float f5 = (f2 * f4) + width2 + (f4 * 4.0f);
                float f6 = i;
                canvas.translate(f5, (f2 * f6) + width3 + (f6 * 4.0f));
                int i3 = (i * 3) + i2;
                float f7 = this.scaleFloats[i3];
                canvas.scale(f7, f7);
    ...
    public ArrayList<ValueAnimator> onCreateAnimators() {
        ...
        int[] iArr2 = {-60, 250, -170, WindowSizeClass.HEIGHT_DP_MEDIUM_LOWER_BOUND, 310, 30, 460, 780, 450};
