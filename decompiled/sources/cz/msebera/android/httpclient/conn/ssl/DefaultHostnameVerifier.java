...
public final class DefaultHostnameVerifier implements HostnameVerifier {
    ...
    private static boolean matchIdentity(String str, String str2, PublicSuffixMatcher publicSuffixMatcher, boolean z) {
        if (publicSuffixMatcher != null && str.contains(InstructionFileId.DOT) && !matchDomainRoot(str, publicSuffixMatcher.getDomainRoot(str2))) {
            return false;
        }
        ...
        if (iIndexOf != -1) {
            ...
            if (strSubstring2.isEmpty() || str.endsWith(strSubstring2)) {
                return (z && str.substring(strSubstring.length(), str.length() - strSubstring2.length()).contains(InstructionFileId.DOT)) ? false : true;
            }
            return false;
        }
