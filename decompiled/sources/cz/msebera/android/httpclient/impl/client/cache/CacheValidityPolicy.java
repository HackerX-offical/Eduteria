...
class CacheValidityPolicy {
    ...
    public boolean isResponseHeuristicallyFresh(HttpCacheEntry httpCacheEntry, Date date, float f2, long j) {
        return getCurrentAgeSecs(httpCacheEntry, date) < getHeuristicFreshnessLifetimeSecs(httpCacheEntry, f2, j);
    }

    public long getHeuristicFreshnessLifetimeSecs(HttpCacheEntry httpCacheEntry, float f2, long j) {
        Date date = httpCacheEntry.getDate();
    ...
    protected boolean contentLengthHeaderMatchesActualLength(HttpCacheEntry httpCacheEntry) {
        if (hasContentLengthHeader(httpCacheEntry)) {
            return httpCacheEntry.getResource() != null && getContentLengthValue(httpCacheEntry) == httpCacheEntry.getResource().length();
        }
        return true;
    }
