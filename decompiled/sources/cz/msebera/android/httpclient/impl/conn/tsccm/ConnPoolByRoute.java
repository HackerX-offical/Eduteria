...
public class ConnPoolByRoute extends AbstractConnPool {
        ...
        try {
            ...
            while (freeEntry == null) {
                ...
                if (this.log.isDebugEnabled()) {
                    this.log.debug(Constants.AES_PREFIX + httpRoute + "] total kept alive: " + this.freeConnections.size() + ", total issued: " + this.leasedConnections.size() + ", total allocated: " + this.numConnections + " out of " + this.maxTotalConnections);
                }
                ...
                if (this.log.isDebugEnabled()) {
                    this.log.debug("Available capacity: " + routePool.getCapacity() + " out of " + routePool.getMaxEntries() + " [" + httpRoute + "][" + obj + Constants.AES_SUFFIX);
                }
                if (z && this.numConnections < this.maxTotalConnections) {
                    ...
                    if (this.log.isDebugEnabled()) {
                        this.log.debug("Need to wait for connection [" + httpRoute + "][" + obj + Constants.AES_SUFFIX);
                    }
    ...
    public void freeEntry(BasicPoolEntry basicPoolEntry, boolean z, long j, TimeUnit timeUnit) {
        ...
        if (this.log.isDebugEnabled()) {
            this.log.debug("Releasing connection [" + plannedRoute + "][" + basicPoolEntry.getState() + Constants.AES_SUFFIX);
        }
        this.poolLock.lock();
        try {
            if (this.shutdown) {
        ...
        while (!z) {
            try {
                ...
                if (basicPoolEntryAllocEntry != null) {
                    if (this.log.isDebugEnabled()) {
                        this.log.debug("Getting free connection [" + routeSpecificPool.getRoute() + "][" + obj + Constants.AES_SUFFIX);
                    }
                    this.freeConnections.remove(basicPoolEntryAllocEntry);
                    if (basicPoolEntryAllocEntry.isExpired(System.currentTimeMillis())) {
                        if (this.log.isDebugEnabled()) {
                            this.log.debug("Closing expired free connection [" + routeSpecificPool.getRoute() + "][" + obj + Constants.AES_SUFFIX);
                        }
                    ...
                    this.log.debug("No free connections [" + routeSpecificPool.getRoute() + "][" + obj + Constants.AES_SUFFIX);
                }
                z = true;
            } finally {
                this.poolLock.unlock();
    ...
    protected BasicPoolEntry createEntry(RouteSpecificPool routeSpecificPool, ClientConnectionOperator clientConnectionOperator) {
        if (this.log.isDebugEnabled()) {
            this.log.debug("Creating new connection [" + routeSpecificPool.getRoute() + Constants.AES_SUFFIX);
        }
    ...
    protected void deleteEntry(BasicPoolEntry basicPoolEntry) {
        HttpRoute plannedRoute = basicPoolEntry.getPlannedRoute();
        if (this.log.isDebugEnabled()) {
            this.log.debug("Deleting connection [" + plannedRoute + "][" + basicPoolEntry.getState() + Constants.AES_SUFFIX);
        }
        this.poolLock.lock();
        try {
            closeConnection(basicPoolEntry);
        ...
        try {
            if (!this.shutdown) {
                ...
                while (it2.hasNext()) {
                    ...
                    it2.remove();
                    if (this.log.isDebugEnabled()) {
                        this.log.debug("Closing connection [" + next2.getPlannedRoute() + "][" + next2.getState() + Constants.AES_SUFFIX);
                    }
