...
public class BasicConnFactory implements ConnFactory<HttpHost, HttpClientConnection> {
    ...
    public HttpClientConnection create(HttpHost httpHost) throws IOException {
        ...
        if (TournamentShareDialogURIBuilder.scheme.equalsIgnoreCase(schemeName)) {
        ...
        if (port == -1) {
            if (httpHost.getSchemeName().equalsIgnoreCase(HttpHost.DEFAULT_SCHEME_NAME)) {
                port = 80;
            } else if (httpHost.getSchemeName().equalsIgnoreCase(TournamentShareDialogURIBuilder.scheme)) {
                port = 443;
            }
        }
