...
public class Parser {
    ...
    public static Packet<String> decodePacket(String str) {
        ...
        if (numericValue >= 0) {
            ...
            if (numericValue < map.size()) {
                if (str.length() > 1) {
                    return new Packet<>(map.get(Integer.valueOf(numericValue)), str.substring(1));
                }
                return new Packet<>(map.get(Integer.valueOf(numericValue)));
            }
        }
        return err;
    }
    ...
    public static Packet decodeBase64Packet(String str) {
        ...
        if (str.charAt(0) == 'b') {
            return new Packet("message", Base64.decode(str.substring(1), 0));
        }
        return decodePacket(str);
    }
