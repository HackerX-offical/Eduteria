...
public abstract class Random {
    ...
    public double nextDouble(double from, double until) {
        ...
        if (Double.isInfinite(d2) && Math.abs(from) <= Double.MAX_VALUE && Math.abs(until) <= Double.MAX_VALUE) {
            double d3 = 2;
            double dNextDouble2 = nextDouble() * ((until / d3) - (from / d3));
            dNextDouble = from + dNextDouble2 + dNextDouble2;
        } else {
            dNextDouble = from + (nextDouble() * d2);
        }
