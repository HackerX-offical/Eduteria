...
public final class LongSaturatedMathKt {
    ...
    public static final long m13843saturatingAddNuflL3o(long j, DurationUnit unit, long j2) {
        Intrinsics.checkNotNullParameter(unit, "unit");
        long jM13779toLongimpl = Duration.m13779toLongimpl(j2, unit);
        if (((j - 1) | 1) == Long.MAX_VALUE) {
            return m13842checkInfiniteSumDefinedPjuGub4(j, j2, jM13779toLongimpl);
        }
        if (((jM13779toLongimpl - 1) | 1) == Long.MAX_VALUE) {
            return m13844saturatingAddInHalvesNuflL3o(j, unit, j2);
        }
        long j3 = j + jM13779toLongimpl;
        return ((j ^ j3) & (jM13779toLongimpl ^ j3)) < 0 ? j < 0 ? Long.MIN_VALUE : Long.MAX_VALUE : j3;
    }

    /* JADX INFO: renamed from: checkInfiniteSumDefined-PjuGub4, reason: not valid java name */
    private static final long m13842checkInfiniteSumDefinedPjuGub4(long j, long j2, long j3) {
        if (!Duration.m13765isInfiniteimpl(j2) || (j ^ j3) >= 0) {
            return j;
        }
    ...
    private static final long m13844saturatingAddInHalvesNuflL3o(long j, DurationUnit durationUnit, long j2) {
        long jM13743divUwyO8pc = Duration.m13743divUwyO8pc(j2, 2);
        long jM13779toLongimpl = Duration.m13779toLongimpl(jM13743divUwyO8pc, durationUnit);
        return (1 | (jM13779toLongimpl - 1)) == Long.MAX_VALUE ? jM13779toLongimpl : m13843saturatingAddNuflL3o(m13843saturatingAddNuflL3o(j, durationUnit, jM13743divUwyO8pc), durationUnit, Duration.m13768minusLRDsOJo(j2, jM13743divUwyO8pc));
    }

    private static final long infinityOfSign(long j) {
        return j < 0 ? Duration.INSTANCE.m13830getNEG_INFINITEUwyO8pc$kotlin_stdlib() : Duration.INSTANCE.m13829getINFINITEUwyO8pc();
    }

    public static final long saturatingDiff(long j, long j2, DurationUnit unit) {
        Intrinsics.checkNotNullParameter(unit, "unit");
        if ((1 | (j2 - 1)) == Long.MAX_VALUE) {
            return Duration.m13784unaryMinusUwyO8pc(infinityOfSign(j2));
        }
    ...
    private static final long saturatingFiniteDiff(long j, long j2, DurationUnit durationUnit) {
        ...
        if (((j3 ^ j) & (~(j3 ^ j2))) < 0) {
            if (durationUnit.compareTo(DurationUnit.MILLISECONDS) < 0) {
                ...
                return Duration.m13769plusLRDsOJo(DurationKt.toDuration(j4, DurationUnit.MILLISECONDS), DurationKt.toDuration(j5, durationUnit));
            }
            return Duration.m13784unaryMinusUwyO8pc(infinityOfSign(j3));
        }
