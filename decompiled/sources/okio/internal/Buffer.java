...
public final class Buffer {
    ...
    */
    ...
    public static final int commonReadUtf8CodePoint(okio.Buffer buffer) throws EOFException {
        ...
        long j = i2;
        if (buffer.size() < j) {
            throw new EOFException("size < " + i2 + ": " + buffer.size() + " (to read code point prefixed 0x" + SegmentedByteString.toHexString(b2) + ')');
        }
    ...
    public static final long commonIndexOf(okio.Buffer buffer, byte b2, long j, long j2) {
        Segment segment;
        int i;
        Intrinsics.checkNotNullParameter(buffer, "<this>");
        long size = 0;
    ...
    public static final long commonIndexOf(okio.Buffer buffer, ByteString bytes, long j) {
        ...
        while (size < size5) {
            ...
            i = (int) ((((long) segment.pos) + j2) - size);
            while (i < iMin2) {
                if (bArr2[i] == b3 && rangeEquals(segment, i + 1, bArrInternalArray$okio2, 1, size4)) {
                }
                i++;
            }
            size += (long) (segment.limit - segment.pos);
