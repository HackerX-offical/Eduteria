    ...
    public int read() throws IOException {
        ...
        if (this.bufPtr > 2 || this.crcFound) {
            ...
            if (ignoreSpace2 == 13 || ignoreSpace2 == 10) {
                ...
                if (ignoreSpace < 0) {
                    armoredInputStream.isEndOfStream = true;
                    return -1;
                }
                if (ignoreSpace == 61) {
                    int iDecode2 = armoredInputStream.decode(readIgnoreSpace(), readIgnoreSpace(), readIgnoreSpace(), readIgnoreSpace(), armoredInputStream.outBuf);
                    armoredInputStream.bufPtr = iDecode2;
                    if (iDecode2 != 0) {
                        throw new IOException("no crc found in armored message.");
                    }
                    int[] iArr = armoredInputStream.outBuf;
                    int i3 = (iArr[2] & 255) | ((iArr[0] & 255) << 16) | ((iArr[1] & 255) << 8);
                    armoredInputStream.crcFound = true;
                    if (i3 == armoredInputStream.crc.getValue()) {
                        return read();
                    }
                ...
                if (ignoreSpace == 45) {
                    do {
                        i = armoredInputStream.in.read();
                        if (i < 0 || i == 10) {
                            break;
                        }
                    } while (i != 13);
                    if (!armoredInputStream.crcFound) {
                        throw new IOException("crc check not found.");
                    }
                    armoredInputStream.crcFound = false;
                    armoredInputStream.start = true;
                    armoredInputStream.bufPtr = 3;
                    if (i < 0) {
                        armoredInputStream.isEndOfStream = true;
                    }
                    return -1;
                }
                iDecode = armoredInputStream.decode(ignoreSpace, readIgnoreSpace(), readIgnoreSpace(), readIgnoreSpace(), armoredInputStream.outBuf);
            } else {
                if (ignoreSpace2 < 0) {
                    this.isEndOfStream = true;
                    return -1;
                ...
                armoredInputStream = this;
                iDecode = armoredInputStream.decode(ignoreSpace2, readIgnoreSpace(), readIgnoreSpace(), readIgnoreSpace(), this.outBuf);
            }
            armoredInputStream.bufPtr = iDecode;
        } else {
            armoredInputStream = this;
        }
        int[] iArr2 = armoredInputStream.outBuf;
        int i4 = armoredInputStream.bufPtr;
        armoredInputStream.bufPtr = i4 + 1;
        int i5 = iArr2[i4];
        armoredInputStream.crc.update(i5);
        return i5;
    }
}
