...
public class Argon2BytesGenerator {
    ...
    private static void fBlaMka(Block block, int i, int i2) {
        block.v[i] = block.v[i] + block.v[i2] + ((block.v[i] & 4294967295L) * (4294967295L & block.v[i2]) * 2);
    }
    ...
    private int getRefLane(Position position, long j) {
        return (position.pass == 0 && position.slice == 0) ? position.lane : (int) ((j >>> 32) % ((long) this.parameters.getLanes()));
    }
