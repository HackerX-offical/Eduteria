...
public class ECIESKeyEncapsulation implements KeyEncapsulation {
    ...
    public CipherParameters decrypt(byte[] bArr, int i, int i2, int i3) throws IllegalArgumentException {
        ...
        return deriveKey(i3, bArr2, eCPointDecodePoint.multiply(d2).normalize().getAffineXCoord().getEncoded());
    }
    ...
    public CipherParameters encrypt(byte[] bArr, int i, int i2) throws IllegalArgumentException {
        ...
        ECPoint[] eCPointArr = {createBasePointMultiplier().multiply(parameters.getG(), bigIntegerCreateRandomInRange), eCPublicKeyParameters.getQ().multiply(this.OldCofactorMode ? bigIntegerCreateRandomInRange.multiply(h2).mod(n) : bigIntegerCreateRandomInRange)};
