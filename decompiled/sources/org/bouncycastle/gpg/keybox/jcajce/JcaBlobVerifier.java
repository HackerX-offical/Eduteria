...
public class JcaBlobVerifier implements BlobVerifier {
    ...
    JcaBlobVerifier(JcaJceHelper jcaJceHelper) throws NoSuchAlgorithmException, NoSuchProviderException {
        MessageDigest messageDigestCreateMessageDigest;
        this.sha1Digest = jcaJceHelper.createMessageDigest("SHA-1");
        try {
            messageDigestCreateMessageDigest = jcaJceHelper.createMessageDigest(StringUtils.MD5);
    ...
    public boolean isMatched(byte[] bArr, byte[] bArr2) {
        ...
        try {
            ...
            throw new IllegalStateException("internal buffer to small: " + e2.getMessage(), e2);
        }
    }
}
