...
public class WTauNafMultiplier extends AbstractECMultiplier {
    ...
    private static ECPoint.AbstractF2m multiplyFromWTnaf(final ECPoint.AbstractF2m abstractF2m, byte[] bArr) {
        ...
        for (int length = bArr.length - 1; length >= 0; length--) {
            ...
            byte b2 = bArr[length];
            if (b2 != 0) {
                abstractF2m3 = (ECPoint.AbstractF2m) abstractF2m3.tauPow(i2).add(b2 > 0 ? preComp[b2 >>> 1] : abstractF2mArr[(-b2) >>> 1]);
                i2 = 0;
            }
        }
    ...
    protected ECPoint multiplyPositive(ECPoint eCPoint, BigInteger bigInteger) {
        ...
        return multiplyWTnaf(abstractF2m, Tnaf.partModReduction(bigInteger, fieldSize, bByteValue, abstractF2m2.getSi(), mu, (byte) 10), bByteValue, mu);
    }
}
