...
public class PGPEncryptedDataGenerator implements SymmetricKeyAlgorithmTags, StreamGenerator {
    ...
    private OutputStream open(OutputStream outputStream, long j, byte[] bArr) throws IllegalStateException, IOException, PGPException {
        ...
        try {
            ...
            if (bArr == null) {
                if (integrityCalculator != null) {
                    ClosableBCPGOutputStream closableBCPGOutputStream = new ClosableBCPGOutputStream(outputStream, 18, ((long) pGPDataEncryptorBuild.getBlockSize()) + j + 25);
                    this.pOut = closableBCPGOutputStream;
                    closableBCPGOutputStream.write(1);
                } else {
                    this.pOut = new ClosableBCPGOutputStream(outputStream, 9, 2 + ((long) pGPDataEncryptorBuild.getBlockSize()) + j, this.oldFormat);
                }
