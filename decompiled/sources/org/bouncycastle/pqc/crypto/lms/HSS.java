...
class HSS {
    ...
    public static HSSPrivateKeyParameters generateHSSKeyPair(HSSKeyGenerationParameters hSSKeyGenerationParameters) {
        ...
        while (i < depth) {
            ...
            h2 *= (long) (1 << hSSKeyGenerationParameters.getLmsParameters()[i].getLMSigParam().getH());
            i++;
            bArr4 = bArr;
        }
        if (h2 == 0) {
        ...
        return new HSSPrivateKeyParameters(hSSKeyGenerationParameters.getDepth(), Arrays.asList(lMSPrivateKeyParametersArr), Arrays.asList(lMSSignatureArr), 0L, h2);
    }
    ...
    public static HSSSignature generateSignature(HSSPrivateKeyParameters hSSPrivateKeyParameters, byte[] bArr) {
        ...
        synchronized (hSSPrivateKeyParameters) {
            ...
            int i = l - 1;
            LMSSignature lMSSignatureGenerateSign = LMS.generateSign(hSSPrivateKeyParameters.getKeys().get(i), bArr);
            LMSSignedPubKey[] lMSSignedPubKeyArr = new LMSSignedPubKey[i];
            int i2 = 0;
            while (i2 < i) {
                int i3 = i2 + 1;
            ...
            hSSSignature = l == 1 ? new HSSSignature(i, lMSSignedPubKeyArr, lMSSignatureGenerateSign) : new HSSSignature(i, lMSSignedPubKeyArr, lMSSignatureGenerateSign);
        }
        return hSSSignature;
    }
