...
public class UTF8 {
    ...
    static {
        ...
        for (int i = 0; i < 128; i++) {
            byte b2 = bArr2[i];
            firstUnitTable[i] = (short) (bArr4[b2] | ((bArr3[b2] & i) << 8));
        }
    }

    private static void fill(byte[] bArr, int i, int i2, byte b2) {
        while (i <= i2) {
            bArr[i] = b2;
            i++;
        }
        ...
        while (i < bArr.length) {
            ...
            if (b2 < 0) {
                ...
                while (b3 >= 0) {
                    ...
                    int i5 = i3 + 1;
                    byte b4 = bArr[i3];
                    i4 = (i4 << 6) | (b4 & Utf8.REPLACEMENT_BYTE);
