...
public class DateTimeZoneBuilder {
    ...
    public DateTimeZoneBuilder addCutover(int i, char c2, int i2, int i3, int i4, boolean z, int i5) {
        if (this.iRuleSets.size() > 0) {
            this.iRuleSets.get(r10.size() - 1).setUpperLimit(i, new OfYear(c2, i2, i3, i4, z, i5));
        }
    ...
    private RuleSet getLastRuleSet() {
        ...
        return dateTimeZoneBuilder.iRuleSets.get(r0.size() - 1);
    }
    ...
    private static final class PrecalculatedZone extends DateTimeZone {
        ...
        static PrecalculatedZone create(String str, boolean z, ArrayList<Transition> arrayList, DSTZone dSTZone) {
            ...
            while (i3 < size) {
                ...
                jArr2[i3] = transition2.getMillis();
                iArr3[i3] = transition2.getWallOffset();
                iArr4[i3] = transition2.getStandardOffset();
            ...
            while (i2 < size - 1) {
                ...
                long j3 = iArr4[i6];
                long j4 = iArr4[i4];
                Period period = new Period(jArr2[i6], jArr2[i4], PeriodType.yearMonthDay(), instanceUTC);
                ...
                if (j != j2 && j3 == j4 && str2.equals(str3) && period.getYears() == 0 && period.getMonths() > 4 && period.getMonths() < 8 && str2.equals(strArr2[2]) && str2.equals(strArr2[4])) {
                    if (ZoneInfoLogger.verbose()) {
                        ...
                        iArr = iArr3;
                        iArr2 = iArr4;
                        System.out.println("     - " + new DateTime(jArr[i6], iSOChronology) + " - " + new DateTime(jArr[i4], iSOChronology));
                    } else {
                        jArr = jArr2;
                        iArr = iArr3;
                        iArr2 = iArr4;
            ...
            if (dSTZone == null || !dSTZone.iStartRecurrence.getNameKey().equals(dSTZone.iEndRecurrence.getNameKey())) {
                ...
                if (dSTZone.iStartRecurrence.getSaveMillis() > 0) {
                    dSTZone3 = new DSTZone(dSTZone.getID(), dSTZone.iStandardOffset, dSTZone.iStartRecurrence.renameAppend("-Summer"), dSTZone.iEndRecurrence);
                } else {
                    dSTZone3 = new DSTZone(dSTZone.getID(), dSTZone.iStandardOffset, dSTZone.iStartRecurrence, dSTZone.iEndRecurrence.renameAppend("-Summer"));
                }
                dSTZone2 = dSTZone3;
            }
            return new PrecalculatedZone(z ? str : "", jArr3, iArr5, iArr6, strArr, dSTZone2);
        }
