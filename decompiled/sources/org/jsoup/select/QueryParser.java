...
public class QueryParser {
    ...
    private static final String[] combinators = {Constants.SEPARATOR_COMMA, ">", MqttTopic.SINGLE_LEVEL_WILDCARD, "~", " "};
    ...
    */
    ...
    private String consumeSubQuery() {
        ...
        while (!this.tq.isEmpty()) {
            if (this.tq.matches("(")) {
                sbBorrowBuilder.append("(").append(this.tq.chompBalanced('(', ')')).append(")");
            } else if (this.tq.matches(Constants.AES_PREFIX)) {
                sbBorrowBuilder.append(Constants.AES_PREFIX).append(this.tq.chompBalanced('[', ']')).append(Constants.AES_SUFFIX);
    ...
    private void findElements() {
        ...
        if (this.tq.matches(Constants.AES_PREFIX)) {
            byAttribute();
            return;
        }
    ...
    private void byTag() {
        ...
        if (strNormalize.startsWith("*|")) {
            this.evals.add(new CombiningEvaluator.Or(new Evaluator.Tag(strNormalize.substring(2)), new Evaluator.TagEndsWith(strNormalize.replace("*|", ":"))));
        } else {
    ...
    private void has() {
        ...
        Validate.notEmpty(strChompBalanced, ":has(selector) sub-select must not be empty");
    ...
    private void containsWholeText(boolean z) {
        ...
        List<Evaluator> list = this.evals;
        if (z) {
            containsWholeText = new Evaluator.ContainsWholeOwnText(strUnescape);
        } else {
            containsWholeText = new Evaluator.ContainsWholeText(strUnescape);
        }
    ...
    private void matches(boolean z) {
        ...
        this.tq.consume(str);
        String strChompBalanced = this.tq.chompBalanced('(', ')');
        Validate.notEmpty(strChompBalanced, str.concat("(regex) query must not be empty"));
        List<Evaluator> list = this.evals;
        if (z) {
    ...
    private void matchesWholeText(boolean z) {
        ...
        List<Evaluator> list = this.evals;
        if (z) {
            matchesWholeText = new Evaluator.MatchesWholeOwnText(Pattern.compile(strChompBalanced));
        } else {
            matchesWholeText = new Evaluator.MatchesWholeText(Pattern.compile(strChompBalanced));
        }
