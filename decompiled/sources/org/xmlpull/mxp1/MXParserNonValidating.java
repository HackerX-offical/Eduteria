...
public class MXParserNonValidating extends MXParserCachingStrings {
    ...
    protected void processAttlistDecl(char c2) throws XmlPullParserException, IOException {
    }

    protected void processEntityDecl(char c2) throws XmlPullParserException, IOException {
    }

    protected char processExternalId(char c2) throws XmlPullParserException, IOException {
        return c2;
    }

    protected void processNotationDecl(char c2) throws XmlPullParserException, IOException {
    }
    ...
    protected void processMarkupDecl(char c2) throws XmlPullParserException, IOException {
        if (c2 != '<') {
            throw new XmlPullParserException(new StringBuffer("expected < for markupdecl in DTD not ").append(printable(c2)).toString(), this, null);
        }
        char cMore = more();
        if (cMore == '?') {
            parsePI();
    ...
    protected void processElementDecl(char c2) throws XmlPullParserException, IOException {
        readName(requireNextS());
        requireNextS();
    }

    protected char readName(char c2) throws XmlPullParserException, IOException {
        if (isNameStartChar(c2)) {
            throw new XmlPullParserException(new StringBuffer("XML name must start with name start character not ").append(printable(c2)).toString(), this, null);
        }
        while (isNameChar(c2)) {
            c2 = more();
        }
